...
<Cmdname sid="anomaly_detection" className="uitext" />는 메트릭스 데이터에서 비정상적인 상승 또는 하락 패턴을 식별해, 평소와 다른 움직임이 감지되면 경고 알림을 발생시키는 기능입니다.
...
이벤트가 특정 시간대(근무/비근무/점검 시간 등)에 이벤트가 동작하도록 태그를 설정하세요. 태그를 설정하지 않으면 이벤트가 활성화된 상태에서 24시간 상시 동작합니다.
