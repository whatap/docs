...
| 조건 | 이벤트 발생 조건 |
| 이벤트 상태가 해소되면 추가 알림 | <span className="vslow">Critical</span>과 <span className="slow">Warning</span> 레벨의 이벤트가 해소되면 정상(RECOVERED) 상태의 알림 수신 여부 <br /> – 활성화: 진행 중 → 정상 상태로 전환되면 RECOVERED 알림을 보내고, 이벤트 기록에서 진행 중/해소 상태를 구분해 볼 수 있음 <br /> – 비활성화: 임계값을 초과할 때 마다 단발성 알림 발생 |
...
- **무음**: 첫 번째 알림이 발생한 후, 선택한 시간(`unset`, `1분`, `5분`, `10분`, `30분`, `1시간`, `12시간`) 동안 같은 이벤트에 대한 추가 알림을 보내지 않습니다.
...
설정한 조건을 과거 데이터에 기반으로 테스트를 진행해, 몇 번의 경고 알림이 발생했는지 확인할 수 있습니다. 

1. 이벤트 규칙을 테스트할 날짜를 선택하고 [ 확인 ] 버튼을 클릭하세요.

1. [ 실행 ] 버튼을 클릭해 알림 발생 건수를 확인하세요.
