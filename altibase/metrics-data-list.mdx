...
| -- | -- | -- |
| `cpu(xos)` | pct | cpu 사용율, `cpu_user` + `cpu_sys` + `cpu_iowait`의 합 |
| `cpu_user(xos)` | pct | cpu가 사용자 코드를 실행하는데 소요한 시간 |
| `cpu_sys(xos)` | pct | cpu가 kernel code를 수행한 시간 |
| `cpu_iowait(xos)` | pct | cpu가 IO를 대기한 시간 |
| `cpu_idle(xos)` | pct | cpu 프로세서 유휴 시간 |
