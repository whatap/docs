...
1. Role의 이름과 설명을 입력하고 Create Role 버튼을 선택해 Summary 화면으로 전환하세요.
1. Maximum session duration 항목의 Edit를 선택해 12시간으로 변경하고 저장하세요. 
1. **Role ARN** 항목을 복사하세요.
1. 와탭 ***관리*** > ***에이전트 설치*** 페이지로 돌아가 ***Role*** ***ARN*** 창에 복사한 **Role ARN**을 붙여넣고 ***확인***을 선택하세요.
