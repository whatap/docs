...
와탭 에이전트는 트랜잭션 정보, SQL과 같은 주요 프로파일 정보에 대해 통계 정보를 수집합니다. 5분마다 목록을 만들고 서버로 전송합니다.
...
:::info
`malloc_sum`은 `trace_malloc_enabled=true`로 변경해야 수집됩니다.(default: `false`)
:::
...
```java title='whatap.conf of [front]'
mtrace_rate=100
mtrace_spec=v1
stat_mtrace_enabled=true
```

```java title='whatap.conf of [api1] & [api2]'
mtrace_spec=v1
stat_mtrace_enabled=true
```
