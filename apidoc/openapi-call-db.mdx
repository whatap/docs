...
* 이 문서에서 작성된 샘플 코드 중 `stime`, `etime`, `oid` 등 파라미터 값은 사용자 환경에 맞게 변경해 이용하세요. 파라미터 값은 다양한 포맷으로 요청할 수 있습니다.
...
| `baseTime`       | <span class='api'>number</span> · <span class='require'>필수</span> | 기준 시간, `yyyyMMdd` 형식 |
| `comparisontime` | <span class='api'>number</span> · <span class='require'>필수</span> | 비교 시간, `yyyyMMdd` 형식 |
