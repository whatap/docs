...
title: Spot 정보 조회(Server)
description: Spot 정보 조회(Server)를 위한 Open API에 대한 설명을 제공합니다.
tags: [ Open API, API, Spot 정보 조회, Server, Server 모니터링 ]
---
...
<span class='apitype'>GET</span> https://service.whatap.io/open/json/sm_proc

```bash title='command'
$ curl -w "\n" -H "x-whatap-token: {{PROJECT API TOKEN}}" -H "x-whatap-pcode: {{projectCode}}" "https://service.whatap.io/open/json/sm_proc"
```
