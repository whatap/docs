...
tags: [ Open API, API, MXQL ]
---

Open API를 이용해 MXQL 데이터를 조회하는 방법을 제공합니다.

:::note
MXQL에 대한 자세한 내용은 [다음 설명서](../mxql/mxql-overview)를 참조하세요.
:::
...
* [다음 설명서](../mxql/mxql-guide#custom-query-saas)에서와 같이 '**SaaS 서비스에서 제공하는 사전에 정의된 MXQL 쿼리 목록**'으로 데이터를 조회하는 방법
...
| '**SaaS 서비스에서 제공하는 사전에 정의된 MXQL 쿼리 목록**'을 사용하는 경우([다음 설명서](../mxql/mxql-guide#custom-query-saas) 참조) | [MXQL path API](#path-api) |
...
```json title='RequestBody example'
{
  "stime": 1639016640000,
  "etime": 1639016940000,
...
| inject | <span class='api'>Optional</span> [다음 설명서](../mxql/mxql-calculate#inject) 참조<br/>예시, `{ "default": "ORDER {key : [oid],  sort : [desc] }"}` |
| param | <span class='api'>Optional</span> [다음 설명서](../mxql/mxql-guide#bind-parameter) 참조<br/> 예시 : `{ "$oidFirst": "520111224", "$oidSecond": "1031483086" }` |
...
```json title='Request Body example (INJECT로 _pk_ 컬럼 추가, param은 사용하지 않음)'
{
  "stime": 1639016640000,
  "etime": 1639016940000,
...
| stime | 데이터 조회 시작 시간 |
| etime | 데이터 조회 끝 시간 |
| mql | [다음 설명서](../mxql/mxql-guide#custom-query-saas) 참조 |
| limit | 최대 데이터 조회 수 |
| pageKey | 문자열 `mxql` 고정값 |
| inject | <span class='api'>Optional</span> [다음 설명서](../mxql/mxql-calculate#inject) 참조<br/>예시 : `{ "default": "ORDER {key : [oid],  sort : [desc] }" }` |
| param | <span class='api'>Optional</span> [다음 설명서](../mxql/mxql-guide#bind-parameter) 참조<br/> 예시 : `{ "$oidFirst": "520111224", "$oidSecond": "1031483086" }` |
