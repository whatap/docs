...
| 구분 | 키             | 설정값  | 비고                                                   |
| ---- | -------------- | ------- | ------------------------------------------------------ |
| 헤더 | `x-whatap-token` | API토큰 | 계정 API토큰: 화면 오른쪽 위 프로필 아이콘 선택 > ***계정 정보 관리*** > ***API토큰*** |

메타성 정보 조회용 OpenAPI로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-meta)를 참조하세요.
...
메타성 정보 조회용 OpenAPI로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-meta#project-info)를 참조하세요.

* 프로젝트 정보 조회(애플리케이션 제품군)
* 프로젝트 정보 및 상세 서버 조회(서버 제품군)
* 프로젝트 에이전트 상태 및 호스트 조회

### Spot 정보 조회

Spot 정보 조회용 Open API로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-apm-spot)를 참조하세요.
...
통계 정보 조회용 OpenAPI로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-apm-stat-data)를 참조하세요.
...
* `stime`: 조회 시작 시간 (UNIX epoch time, millisecond)
* `etime`: 조회 종료 시간 (UNIX epoch time, millisecond)<br/>
...
프로젝트 통계 조회용 OpenAPI로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-project-stat)를 참조하세요.
...
장기 통계 조회용 OpenAPI로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-long-stat)를 참조하세요.
...
단기 조회용 Open API로 호출할 수 있는 항목은 다음과 같습니다. Open API 호출 사용법은 [다음 설명서](openapi-call-short-stat)를 참조하세요.
