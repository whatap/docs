...
```bash text title='{Tomcat 설치 경로}/bin/catalina.sh' showLineNumbers
########## WHATAP START ############
WHATAP_HOME=/whatap
...
:::note
```ls ${WHATAP_HOME}/whatap.agent-X.Y.Z.jar | sort | tail -1``` 항목은 향후 에이전트 업데이트를 진행할 경우 최신 와탭 에이전트를 적용하기 위해서입니다.
:::
