...
:::note
whatap.agent-2.1.0 버전 이상부터는 애플리케이션 서버를 통합 모니터링하는 로그싱크(LogSink) 기능을 제공합니다. 로그싱크 설정 옵션에 대한 자세한 설명은 [부가기능 → 로그싱크(LogSink)](java-agent-adv-settings#logsink)를 참조하세요.
:::
