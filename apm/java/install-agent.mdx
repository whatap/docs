...
<details>
<summary>JAVA 에이전트 설치 파일 구성</summary>

Java 에이전트 파일은 애플리케이션 모니터링에 필요한 정보를 추출해 와탭 수집 서버로 전달하는 트레이서와 트레이서가 데이터를 보낼 수 있도록 돕는 요소로 구성되어 있습니다.

* *whatap.conf*<br/>
  모니터링 에이전트의 설정 정보, 프로젝트 액세스 키, 와탭 데이터 수집 서버의 IP 정보 등을 저장하는 설정 파일입니다. 자세한 내용은 [다음 문서](java-set-agent)를 참조하세요.
