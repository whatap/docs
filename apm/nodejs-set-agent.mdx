...
:::note
Node.js 에이전트에서 설정할 수 있는 더 많은 옵션에 대한 설명은 [다음 설명서](nodejs-set-agent-adv)을 참조하세요.
:::
...
whatap.conf 파일에 에이전트 이름과 관련한 자세한 내용은 [다음 설명서](nodejs-set-agent-adv#agentname)를 참조하세요.
