...
Elastic Beanstalk Configuration files(*.ebextensions*)를 통해서 설치하세요.

*.ebextensions* 디렉터리에 아래 *config* 파일을 추가한 후 신규 버전을 배포하세요.

`commands` 명령어를 실행하세요.

1. whatap 리포지터리를 설치하세요.

1. whatap-php 패키지를 설치하세요.

1. */usr/whatap/php/install.sh*를 통해서 PHP extension을 설정하세요.

프로젝트 액세스 키와 서버 아이피는 ***프로젝트 관리*** > ***에이전트 설치*** 페이지에서 발급받고 확인할 수 있습니다.
