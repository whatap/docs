...
1. php-fpm 실행 명령어 링크를 생성하세요.
  ...
  ```
1. whatap 저장소 및 whatap-php 패키지를 설치하세요.
  ```docker
...
1. PHP 확장 모듈 및 whatap-php 서비스를 설정하세요.
  ```docker
  #RUN (echo "[ 발급된 프로젝트 액세스 키 ]"; echo "[ 발급된 서버 IP ]")|/usr/whatap/php/install.sh
  RUN (echo "xxxxxxxx"; echo "1.1.1.1/2.2.2.2")|/usr/whatap/php/install.sh
  ```
  :::note
  PHP 모니터링 프로젝트 생성 후 **프로젝트 액세스 키**와 서버 IP가 발급됩니다. */* 구분자로 여러 개의 IP 발급이 가능합니다.
  :::
