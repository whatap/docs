...
<!-- 개요 페이지로 옮길지 고민 -->
설정 스크립트를 통해서 트레이서는 **PHP 확장 모듈**로 등록되고, 에이전트는 **whatap-php 서비스**로 실행됩니다.
...
PHP 에이전트 파일은 애플리케이션 모니터링에 필요한 정보를 추출하여 와탭 수집 서버로 전달하는 트레이서, 수집된 정보를 서버에 전송하기 위한 에이전트, 트레이서와 에이전트를 서버에 동적으로 적용하기 위한 설치 스크립트 파일로 구성됩니다.
...
* *whatap.ini*  
    애플리케이션 서버의 데이터를 수집하는 PHP 확장 모듈(PHP Extension module)의 설정 정보, 수집 서버의 주소와 서버의 프로젝트 프로젝트 액세스 키가 입력되는 파일입니다.

* *whatap-install-#.log*  
    설치 과정에 대한 로그 파일입니다. (*/usr/whata/php/logs*)

* *whatap-boot-#.log*
    에이전트 로그 파일입니다. (*/usr/whata/php/logs*)
