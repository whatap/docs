...
서비스에 영향을 주지 않는 환경에서 Apache 및 PHP-FPM에 `Core dump` 설정을 추가한 후 *whatap.so*를 다시 로딩합니다. 이후 오류가 발생하면 *dump* 파일이 생성됩니다. *dump* 파일을 gdb로 분석하여 원인을 해결하세요. 

### Core Dump 설정

*dump* 파일을 생성할 디렉토리의 권한을 부여합니다.
...
*dump* 파일이 생성되면 gdb를 통해 오류 원인을 확인합니다. `bt full` 명령어를 통해서 전체 스택 정보를 확인합니다.