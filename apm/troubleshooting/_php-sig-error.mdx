...
Apache conf 에 CoreDumpDirectory 항목을 설정하세요. 
...
```
Apache를 재시작하면 설정이 완료됩니다. 오류가 발생하면 *dump* 파일이 생성됩니다.

*dump* 파일이 생성되면 gdb를 통해 오류 원인을 확인합니다. `bt full` 명령어를 통해서 전체 스택 정보를 확인하세요.

```b# gdb /usr/sbin/httpd /home/httpd-core/core-httpd.31832
...
...
...
FPM 설정 파일에서 `rlimit_core` 를 설정하세요. 

```bash
rlimit_core = unlimited
```

PHP-FPM을 재시작하면 설정이 완료됩니다. 오류가 발생하면 *dump* 파일이 생성됩니다.

*dump* 파일이 생성되면 gdb를 통해 오류 원인을 확인하세요. `bt full` 명령어를 통해서 전체 스택 정보를 확인하세요.
