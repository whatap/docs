...
본 문서는 VPC 환경에서 WhaTap Forwarder를 사용하기 위한 추가 설정을 안내합니다. 먼저 **WhaTap Forwarder**를 설치하세요. 자세한 내용은 [다음 문서](../aws-log/install-aws-log#install-whatap-forwarder)를 참조하세요.

:::

**WhaTap Forwarder**는 AWS Cloudwatch LogGroup과 S3에 쌓인 AWS Log를 수집합니다. 설치는 AWS Lambda Function으로 진행합니다. AWS Lambda는 기본적으로 **VPC**(Virtual Private Cloud)에서 시작되지 않습니다. 하지만 다음과 같은 추가 설정을 통해 **WhaTap Forwarder**를 **VPC** 환경에서 시작할 수 있습니다.
