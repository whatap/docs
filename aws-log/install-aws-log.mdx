...
* Connection Timeout: 와탭 수집 서버로의 로그 전송 타임아웃입니다.
...
로그 전송용 **WhaTap Forwarder**를 AWS Lambda Function으로 설치하세요. 설치 과정은 AWS CloudFormation에서 진행합니다. 다음의 설치 입력 정보가 필요합니다.
...
* **ConnectionTimeOut**<br/>
  기본값 `10`<br/>
  WhaTap Forwarder가 와탭 수집 서버에 접속 시 타임 아웃 시간(초)을 지정합니다.
* **MemorySize**<br/>
  기본값 `1024`<br/>
...
1. ***WhaTap Forwarder 설치 페이지*** 버튼을 선택하세요. 
1. CloudFormation 실행 페이지로 이동 후 설치 옵션을 지정하세요. 
1. ***Create stack*** 버튼을 선택하세요. 설치에 약 2분이 소요됩니다.

### WhaTap Forwarder의 ARN 확보

1. 상단 오른쪽의 ***갱신*** 버튼을 선택하세요. 스택의 생성 진행 상태를 확인할 수 있습니다. 
  :::note

  스택 설치의 최종 단계인 WhaTapAWSLog 생성이 진행 중이라면 **WhaTap Forwarder**의 **ARN**을 확보할 수 있습니다. 

  :::
1. 하단 오른쪽의 ***aws-log-demo-whatapforwarder-…*** 을 선택하세요. **WhaTap Forwarder**의 상세 화면으로 이동합니다.
