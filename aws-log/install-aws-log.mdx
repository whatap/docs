...
1. 와탭 ***WhaTap Forwarder 설치*** 탭에서 ***AWS Region***을 선택하세요.
1. ***WhaTap Forwarder 설치 페이지*** 버튼을 선택해 CloudFormation 실행 페이지로 이동하세요.
1. CloudFormation 실행 페이지에서 설치 옵션을 지정하세요. 기본 파라미터는 입력되어있습니다.
1. 하단의 ***Create stack*** 버튼을 선택하세요. 설치에 약 2분이 소요됩니다.
...
1. 생성된 IAM 역할을 지정해 구할 수 있는 AWS 자원을 조회하세요.
