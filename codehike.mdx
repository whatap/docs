...
강조하려는 코드 윗줄에 `// !mark(1:5)`와 같은 방식으로 주석을 추가하세요. `// !mark(1:5) blue`와 같은 방식으로 색상을 추가할 수 있습니다.

:::note

지원하는 언어는 `js`, `ts` 형식입니다.

> <code>```js title="Windows Powershell" showLineNumbers</code>

:::

```js title="Windows Powershell" showLineNumbers
# 관리자 권한 필요
