...
IP 트랜잭션 통계를 수집하기 위해 다음 옵션 설정이 필요합니다. 기본값은 `false` 입니다.
...
| <Cmdname sid="type_oid" /> | 에이전트 이름 |
| <Cmdname sid="type_onode" /> | `whatap.onode`로 분류된 에이전트 그룹 단위 |
| <Cmdname sid="type_okind" /> | `whatap.okind`로 분류된 에이전트 그룹 단위 |
| <Cmdname sid="TTL09049" /> | 조회 기간 중 트랜잭션의 에러가 최초 생성된 통계시간(5분 또는 1시간) |
...
| <Cmdname sid="TTL06915" /> | HTTP 호출 시간에 대한 표준 편차 |


## 클라이언트 IP 지역별 호출 건수 {#client_tx_stat}
