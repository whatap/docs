...
<Cmdname sid="topstack" className="uitext" />을 클릭하면 해당 최상위 스택에 대한 호출 빈도를 확인할 수 있습니다. <Cmdname sid="topstack" className="uitext" />의 호출 관계는 1 대 1 관계이므로 <Cmdname sid="topstack" className="uitext" />의 depth가 밑으로 내려갈수록 정보의 정확성이 떨어질 수 있습니다. 하위 depth에 대한 정보는 참고 용도로 사용하며 튜닝을 진행하세요.
