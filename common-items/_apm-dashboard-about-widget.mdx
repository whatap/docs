...
#### 액티브 HTTPC/SQL {#active-httpc-sql}
...
* Normal: 0초 ~ 3초
    * 보라색(<span class='purple'>HTTPC Normal</span>)
    * 민트색(<span class='mint'>SQL Normal</span>)
...
위젯의 핵심 시각화 요소로, 진행 중인 HTTPC/SQL을 호스트/데이터베이스 URL로 분할하여 표시합니다. 전체 트랜잭션에서 문제를 발견했다면 이 차트를 통해 구체적인 외부 호출 병목을 확인할 수 있습니다.
