## 다음 단계

<div className="nextstep">

* **오픈 소스 추적**

  애플리케이션에서 사용하는 프레임워크나 오픈소스 라이브러리에 따라 에이전트 옵션을 추가해야 할 수 있습니다. 예를 들어, 프레임워크나 오픈소스로 spring-boot-3.x, feign-client-11, okhttp3-4.4 사용 시 다음과 같이 옵션을 설정하세요.

  ```ini title='whatap.conf'
  weaving=spring-boot-3.0,feign-11,okhttp3-4.4
  ```

  사용자의 애플리케이션 환경에 맞는 프레임워크나 오픈소스 라이브러리를 설정하는 방법은 [다음 문서](../agent-weaving)를 참조하세요.

* **설치 점검하기**

  프로젝트 생성 및 에이전트 설치, JVM 옵션까지 모두 적용했다면 [다음 문서](../install-check)에서 점검 사항을 확인하세요.

* **설치 문제 해결**

  에이전트 설치 시 발생할 수 있는 다양한 문제들과 이를 해결하기 위한 구체적인 지침을 제공합니다. 방화벽 설정, SpringBoot 및 Tomcat JMX 설정, OSGI 프레임워크 사용 서버 설정 등 다양한 상황에 대한 해결책을 안내합니다. 자세한 내용은 [다음 문서](../agent-troubleshooting)를 참조하세요.

* **에이전트 설정**

  에이전트 설정(*whatap.conf*) 파일에 옵션을 적용해 모니터링을 위한 다양한 기능을 제공합니다. 기본 설정부터 서버 연결 및 데이터 전송 설정, 여러 애플리케이션 서버의 설정 파일 관리 방법, 트랜잭션 추적 등을 포함합니다. 자세한 내용은 [다음 문서](../set-agent)를 참조하세요.

* **모니터링 시작하기**

  모든 설정을 완료한 다음 애플리케이션 서버를 다시 시작하세요. 에이전트가 정보를 수집하기 시작합니다. 먼저 <Cmdname sid="side_appDashboard" className="uitext" /> 메뉴에서 모니터링 데이터가 수집되는지 확인하세요. <Cmdname sid="side_appDashboard" className="uitext" />에 대한 자세한 내용은 [다음 문서](../dashboard)를 참조하세요.

</div>