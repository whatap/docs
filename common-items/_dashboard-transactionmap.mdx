...
<Cmdname sid="transaction_map" className="uitext" /> 차트는 종료된 개별 트랜잭션의 응답시간을 분포도 형태로 표현한 차트입니다. 히트맵과 동일하게 분포 패턴에 따른 문제점을 발견하고 분석할 수 있습니다. <Cmdname sid="hitmap" className="uitext" />은 5분 단위로 트랜잭션을 그룹화해서 보여주지만 <Cmdname sid="transaction_map" className="uitext" />은 트랜잭션을 개별로 표시합니다.
