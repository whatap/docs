...
DBXC는 안정적인 실행과 직관적인 사용자 경험, 다양한 모니터링 기능을 제공합니다.

| 특징 | 설명 |
| --- | --- |
| **안정성** | - PID 파일 기반 중복 실행 방지 <br /> - 헬스체크를 통한 정상 시작 확인 <br /> - 5초 시작 타임아웃 설정 |
| **사용자 경험** | - 프로젝트(agent)별 일관된 색상 코딩 <br /> - 실시간 로그 스트리밍 <br /> - 직관적인 상태 표시 |
| **모니터링** | - HTTP 서버 기반 상태 API (포트: 53200) <br /> - 구조화된 로그(Zap 라이브러리) <br /> - 로그 파일 자동 회전 |
