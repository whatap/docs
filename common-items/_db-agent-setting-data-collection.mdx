...
* **slave_ip** <span class='type'>string</span>
  ...
  IP 주소 확인은 `select inet_server_addr();`로 합니다. 마스터 노드에서 `pg_stat_replication.client_addr`를 조회했을때 나오는 ip중 하나여야 합니다. (RDS의 경우 ip대신 pg_replication_slots.slot_name으로 master/slave관계가 셋팅되므로 slave_ip를 설정하지 않아도 됩니다)
