...
* **connect_timeout** <span class='type'>Int</span>
  ...
  수집 쿼리가 지정된 시간을 초과하면, 해당 쿼리에 대한 데이터 수집을 중지합니다.
...
* **db2_opt** <span class='type'>Int</span>
  ...
  * `0`: `activity_state`가 **EXECUTING** 상태인 세션만 수집합니다.

  * `1`: `activity_state`가 **EXECUTING**, **RECEIVING**, **SUSPENDED** 상태인 세션을 모두 수집합니다.
...
* **sqlstat_send_interval** <span class='type'>Int</span>
  ...
  SQL 통계 데이터를 생성하고 전송하는 주기를 설정합니다.
  1분 또는 5분 으로 설정할 수 있으며, 보다 정밀한 분 단위 통계 분석이 필요한 경우 1로 설정하면 됩니다.
  이 설정은 Top SQL 화면에서 1분 단위의 상세한 분석을 가능하게 해줍니다.
...
* **include_cpu_io** <span class='type'>Boolean</span>
  ...
  CPU 수집 시 `io_busy` 값을 포함할지 여부를 설정합니다.

* **check_suspect_db_interval** <span class='type'>Second</span>

  기본값 `60` (단위: 초)

  Suspect DB를 수집하는 주기를 설정합니다.
