...
* **tables_min_size** <span class='type'>Int</span>
  ...
  테이블 정보를 수집할 때 설정한 값보다 작은 크기의 테이블 정보는 수집하지 않습니다.
...
* **slave_ip** <span class='type'>string</span>
  ...
  IP 주소 확인은 `select inet_server_addr();`로 합니다. 마스터 노드에서 `pg_stat_replication.client_addr`를 조회했을 때 나오는 IP 주소 중 하나여야 합니다.
