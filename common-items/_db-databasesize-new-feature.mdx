...
전체 데이터베이스 사이즈는 `pg_database_size` 함수로 조회하며 모니터링 계정에 connect 권한 또는 `pg_read_all_stats` 권한이 있어야 합니다. `template data`는 수집에서 제외됩니다. 
...
테이블별 **DATA SIZE**, **INDEX SIZE**를 표시합니다. PostgreSQL은 테이블 사이즈 정보 수집을 위해 DBX 에이전트에 다음 옵션을 설정해야 합니다.
