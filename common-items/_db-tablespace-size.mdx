...
<Xclude product="altibase,tibero,db2">

* 상위 50개의 세그먼트 크기를 확인할 수 있습니다.

</Xclude>

테이블스페이스 사용량을 실시간으로 모니터링하여 용량을 관리하고, 사용률이 높은 테이블스페이스와 세그먼트를 추적해 용량 관리와 성능을 최적화하세요. 과도한 사용도 사전에 감지하고 대응할 수 있습니다.
...
테이블스페이스 사이즈 데이터를 수집하려면 에이전트 옵션을 설정해야 합니다.
...
테이블스페이스 사이즈 데이터를 수집하려면 데이터베이스 권한과 에이전트 옵션을 설정해야 합니다. 세그먼트 사이즈 데이터는 하루 한 번 수집하며, 아래 절차에 따라 설정을 완료하세요.

### 조회 권한 부여하기

세그먼트 사이즈 데이터를 수집하려면 모니터링 사용자에게 적절한 데이터베이스 권한을 부여해야 합니다. 다음 SQL 명령어로 권한을 설정하세요.
...
에이전트 옵션에서 테이블스페이스 사이즈 데이터의 수집 기준을 설정할 수 있습니다.
...
에이전트 옵션에서 세그먼트 사이즈 데이터의 수집 기준을 설정할 수 있습니다.
...
<Xclude product="altibase,db2"><ProdImg img='db-stat-table-space.png' desc='Tablespace size' /></Xclude>

<InDoc product="db2"><ImgLang img="db-table-space-size-db2.png" desc="Tablespace size" /></InDoc>
...
* <Cmdname sid="table_space_total" className="uitext" />: 테이블스페이스의 전체 크기

* <Cmdname sid="table_space_used" className="uitext" />: 현재 사용 중인 크기

* <Cmdname sid="table_space_usage" className="uitext" />: 전체 크기 대비 사용 비율

* <Cmdname sid="table_space_changed_amount" className="uitext" />: 최초 조회한 시간 대비 현재 시간의 사용량 변화
...
최초 조회한 시간과 현재 시간을 비교하여 각 테이블스페이스의 사용량 변화를 확인할 수 있습니다. 테이블스페이스의 세부 내역을 확인하려면 테이블스페이스 이름 옆의 ![화살표 아이콘](/img/ic-caret-right.svg) 아이콘을 클릭하세요 Altibase에서 제공하는 테이블스페이스 및 데이터 파일 사이즈 정보는 `MAX`, `ALLOC`, `CURR`, `USED` 등 다양한 값을 포함합니다. 화면에 표시된 값은 이러한 항목을 기반으로 구성했습니다.

### ![](/img/number-01.png) TABLESPACE TYPE

DISK, MEMORY, TEMP, UNDO 등 `V$TABLESPACES.TYPE` 별로 테이블스페이스의 사이즈를 확인할 수 있습니다.

* <UI>TOTAL</UI>: 테이블스페이스의 `TOTAL` 크기의 합계

* <UI>USED</UI>: 테이블스페이스의 `USED` 크기의 합계

* <UI>USAGE</UI>: (`USED` / `TOTAL`) 🞨 100으로 계산된 사용 비율

### ![](/img/number-02.png) TABLESPACE

하나의 테이블스페이스는 여러 개의 데이터 파일로 구성됩니다.

* <UI>TOTAL</UI>: 데이터 파일의 `TOTAL` 크기(`V$DATAFILES.CURRSIZE` 🞨 `PAGESIZE`)의 합계

* <UI>USED</UI>: `X$SEGMENT.TOTAL_USED_SIZE`를 기반으로 계산된 사용된 크기
...
* <Cmdname sid="TTL08165" className="uitext" />: 세그먼트가 속한 컨테이너 이름

* <Cmdname sid="TTL08166" className="uitext" />: 세그먼트가 속한 테이블스페이스 이름

* <Cmdname sid="TTL08167" className="uitext" />: 세그먼트를 소유하고 있는 사용자 또는 스키마

* <Cmdname sid="TTL08168" className="uitext" />: 세그먼트의 이름, <Cmdname sid="TTL08168" className="uitext" /> 선택 시 오브젝트 상세 정보를 확인 가능

* <Cmdname sid="TTL08169" className="uitext" />: 세그먼트의 유형 (예: TABLE, INDEX)

* <Cmdname sid="TTL08177" className="uitext" />: 해당하는 경우 세그먼트의 파티션 이름

* <Cmdname sid="table_space_size" className="uitext" />: 세그먼트의 크기
...
* ***Tablespace Type***: 테이블스페이스의 유형

* ***Tablespace***: 해당 세그먼트가 속한 테이블스페이스의 이름, 테이블스페이스는 데이터베이스에서 데이터를 저장하는 논리적인 공간

* ***Table***: 해당 세그먼트가 속한 테이블의 이름, 어떤 테이블이 공간을 차지하고 있는지 식별 가능함

* ***User***: 해당 세그먼트를 소유하고 있는 데이터베이스 사용자 계정의 이름, 어떤 사용자가 해당 데이터를 관리하는지 파악 가능함

* ***Segment Type***: 세그먼트의 유형

* ***Partition***: 파티셔닝된 테이블이나 인덱스의 경우 해당 세그먼트가 속한 파티션 정보를 표시, 파티셔닝은 큰 테이블을 더 작은 단위로 나누어 관리하는 방법

* ***mbytes***: 해당 세그먼트가 차지하고 있는 메가바이트(MB) 단위의 공간 크기입니다. 이를 통해 각 세그먼트가 사용하고 있는 물리적 저장 공간의 양을 확인 가능함
