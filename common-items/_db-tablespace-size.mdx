...
* ***Partition***: 파티셔닝된 테이블이나 인덱스의 경우 해당 세그먼트가 속한 파티션 정보 표시, 파티셔닝은 큰 테이블을 더 작은 단위로 나누는 관리 방법

* ***mbytes***: 해당 세그먼트가 차지하고 있는 메가바이트(MB) 단위의 공간 크기 이를 통해 각 세그먼트가 사용하고 있는 물리적 저장 공간의 양을 확인 가능함
