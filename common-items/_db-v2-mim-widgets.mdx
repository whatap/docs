...
* ***SQL Elapse Map***
  ...
  쿼리의 실행 시간은 액티브 세션을 수집하여 계산합니다. 따라서 액티브 세션 수집 주기에 따라 오차 범위가 발생할 수 있습니다. 다음 그림을 참고하세요. 수집 주기 사이에 종료된 SQL 수행 시간은 계산되지 않습니다. 수집 주기가 짧을수록 오차 범위는 줄어들 수 있지만 DB 부하를 고려해 일반적으로 5초로 세팅했습니다. 장시간 수행되는 쿼리 모니터링에 유용합니다. ***SQL 통계*** 메뉴의 데이터도 동일합니다.
...
* ***Index Physical Reads*** (`index_physical_reads`)
  ...
  데이터베이스에서 디스크를 통해 읽어진 인덱스 페이지 수입니다. 디스크 I/O 성능을 평가하는 데 중요한 지표로, 높은 값은 디스크 작업이 증가했음을 의미합니다. 디스크 I/O 병목 현상을 식별하거나, 인덱스 효율성을 분석하는 데 사용됩니다.

* ***Logical I/O*** (`data_logical_reads`)

  데이터베이스의 버퍼 풀에서 데이터를 읽은 총 페이지 수입니다. 시스템의 메모리 내 캐싱 동작을 보여주는 지표로, 높은 값은 버퍼 풀이 효율적으로 동작하고 있음을 나타낼 수 있습니다. 논리적 읽기와 물리적 읽기를 비교해 버퍼 풀의 효율성을 분석합니다.

* ***Physical I/O*** (`data_physical_reads`)

  데이터베이스에서 디스크를 통해 데이터를 읽은 총 페이지 수입니다. 물리적 I/O 활동을 보여주는 지표로, 버퍼 풀이 데이터를 캐싱하지 못한 경우 증가할 수 있습니다. 디스크 I/O 병목 문제를 파악하고, 물리적 디스크 접근을 줄이는 방향으로 튜닝할 수 있습니다.

* ***Data Hit Ratio(%)*** (`data_hit_ratio`)

  논리적 읽기 대비 물리적 읽기의 비율로, 데이터가 버퍼 풀에 캐싱된 비율입니다. 높은 값은 대부분의 데이터 읽기 작업이 메모리 내에서 처리되고 있음을 나타냅니다. 데이터베이스 성능 최적화를 위해 버퍼 풀 크기 및 정책을 분석할 때 사용합니다.

* ***Index Hit Ratio(%)*** (`index_hit_ratio`)

  논리적 인덱스 읽기 대비 물리적 인덱스 읽기 비율입니다. 높은 값은 인덱스가 메모리에서 효율적으로 처리되고 있음을 의미합니다.

* ***Total Hit Ratio(%)*** (`total_hit_ratio`)

  전체 데이터와 인덱스 읽기에 대한 논리적 읽기 대비 물리적 읽기 비율입니다. 데이터베이스 전체의 캐싱 효율성을 평가하는 핵심 지표입니다. 데이터베이스 운영 환경의 전반적인 캐싱 성능을 분석하고, 튜닝의 우선순위를 정하는 데 활용할 수 있습니다.
