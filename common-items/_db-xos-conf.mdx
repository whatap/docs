...
- **port_check** <span class='type'>String</span>
  ...
  XOS 에이전트에서 옵션값으로 지정한 포트로 직접 접속을 시도한 후, 실패 시 해당 포트 리스트를 `db_port_check`로 전송합니다.

  ```ini title="xos.conf"
  port_check=192.168.122.113:3333,192.168.122.113:3334
  ```

  지정한 포트로 접속이 안될 경우 <Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" /> 메뉴에서 다음과 같이 확인할 수 있습니다.
...
- **process_check** <span class='type'>String</span>
  ...
  해당 문자열을 포함한 프로세스가 모니터링되지 않을 때, `db_process_check`로 데이터를 전송합니다. 예를 들어, **melong**, **haha** 2개 프로세스가 없을 경우 <Cmdname sid="side_analysis" className="uitext" /> &gt; <Cmdname sid="side_metricsSearch" className="uitext" /> 메뉴에서 다음과 같이 확인할 수 있습니다.
...
- **iplist_interval** <span class='type'>Second</span>
  ...
  *whatap.conf* 파일에 `xos_dbip=true` 옵션을 설정하면 `db_ip_list`로 전송합니다.
