...
| `spid`          | OS 프로세스 ID                                                   | -    |
| `username`      | 세션을 실행 중인 데이터베이스 사용자 이름                                | -    |
| `status`        | 세션 상태 (ACTIVE, INACTIVE, KILLED 등)                          | -    |
| `program`       | 세션을 생성한 OS 프로그램 이름                                       | -    |
| `module`        | 세션을 생성한 클라이언트 모듈 이름 (예: 애플리케이션 이름)                  | -    |
...
| `event`         | 현재 세션이 대기 중인 이벤트 이름 (예: `enq: TX - row lock contention`) | -    |
| `id1`           | 락 식별자 1 (락 대상 오브젝트를 식별하는 데 사용됨, 예: 트랜잭션 ID 등)       | -    |
| `id2`           | 락 식별자 2 (락의 세부 정보나 버전 정보를 나타냄)                         | -    |
| `last_call_et`  | 마지막 호출 이후 경과 시간. 대기 상태라면 현재 대기 시간 의미                 | 초   |
| `sql_id`        | 현재 실행 중인 SQL의 ID                                             | -    |
| `sql_text`      | 현재 실행 중인 SQL의 텍스트                                           | -    |
| `prev_sql_id`   | 현재 실행 중인 SQL 이전에 실행된 SQL의 ID                               | -    |
| `prev_sql_text` | 이전 SQL의 텍스트                                                   | -    |
