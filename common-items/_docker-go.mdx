...
Docker 컨테이너 기반의 Go 애플리케이션에 WhaTap 모니터링 에이전트를 설치하는 방법을 안내합니다.
...
- [whatap-go-inst 가이드](auto-instrument-guide.md) - whatap-go-inst 상세 사용법
- [API 가이드](api-guide.md) - go-api 직접 사용이 필요한 경우
- [설치 가이드](install-agent.md) - 일반 Linux 환경 설치


{/* 필요한지 확인하고 넣기 */}
{/* ## whatap-go-inst 작동 원리

`whatap-go-inst` 도구는 빌드 시 자동으로 모니터링 코드를 삽입합니다.
...
#### 3. 데이터베이스 래핑
```go
db, _ := whatapsql.Open("mysql", dsn)  // sql.Open → whatapsql.Open
```
...
| 분류 | 지원 라이브러리 |
|------|----------------|
| **웹** | Gin, Echo, Fiber, Chi, Gorilla Mux, FastHTTP, net/http |
| **DB** | database/sql, sqlx, GORM v1/v2 |
