...
* <Cmdname sid="simulation" className="uitext" />: 이벤트 조건을 설정한 후, <Cmdname sid="simulation" className="uitext" /> 버튼을 클릭하면 설정한 이벤트의 결과를 예측할 수 있습니다.
...
* ***이벤트 동작 시간***: 이벤트 동작 시간 태그를 설정하면 해당 태그를 가진 알림은 해당 시간대에만 이벤트가 동작합니다.  
***+ 추가***를 클릭하면, ***이벤트 동작 시간***에서 태그 목록을 확인하고 새로운 태그를 생성할 수 있습니다. 

  * 태그 생성은 ***이벤트 동작 시간***의 ***+ 새로운 태그 생성***을 클릭해 태그 이름, 요일, 시간, 색상을 선택하고 적용 버튼을 클릭합니다.

  * 태그 수정 및 삭제는 태그 목록에서 ![편집 아이콘](/img/ico-edit.svg)을 클릭합니다.

  <ImgLang img="event-operation-time.png" desc="Resolved notification" />

  :::warning

  시간 태그를 삭제하면 적용된 모든 알림에서 해당 태그가 삭제됩니다. 
...
<ImgLang img='event-config-v2-target-db2.png' desc='이벤트 대상 선택' />
