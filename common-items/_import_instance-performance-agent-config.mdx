...
모니터링 대상 서버에 위치한 *whatap.conf* 파일을 직접 수정하지 않고 에이전트 설정 옵션을 추가하거나 수정, 삭제할 수 있습니다. <Cmdname sid="BTN08153" className="uitext" /> 버튼 클릭 시 선택한 에이전트의 *whatap.conf* 파일을 다운로드합니다.
