...
### AI 스레드 분석 {#ai-thread-analysis}

스레드 덤프 상세 화면에서 ![AI 아이콘](/img/ico-chatbot.svg) <Cmdname sid="BTN10229" className="uitext" /> 버튼을 클릭합니다.  

AI 기반 분석 결과는 드로워 형태로 제공됩니다.  
...
### 스레드 분석 재시도
드로워 상단 ![새로고침 아이콘](/img/ic-refresh2.svg) 버튼을 클릭하여 동일한 스레드 덤프를 다시 분석할 수 있습니다.  

분석 결과는 와탭 페이지에 설정된 언어로 제공됩니다. 일부 경우 영어로 표시될 수 있습니다.  

프로젝트당 하루 20회까지 분석할 수 있으며, 프로젝트 타임존 기준 매일 00:00:00에 초기화됩니다.

- 분석 회수 차감 기준  
  - 새로운 스레드 덤프 분석 시 1회 차감  
    - 애플리케이션 선택 상태 변경  
      - 애플리케이션 리스트에서 다른 애플리케이션 선택  
    - 시간값 변경  
      - 다른 애플리케이션 선택 시 (선택 상태와 시간 변경)  
      - 동일한 애플리케이션 재선택 시 (선택 상태 유지, 시점 변경으로 덤프 변경)  
  - 동일 스레드 덤프에 대해 드로워 내에서 재분석 시도 시에도 1회 차감  
