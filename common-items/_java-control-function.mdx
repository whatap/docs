...
* **_enable_emb_tools** <span class='type'>Boolean</span>

  기본값: `true`

  JVM 메모리에 올라가 있는 Heap 점유 객체 현황(힙 메모리상의 객체별 사이즈)을 조회합니다. 조회된 결과는 <Cmdname sid="app_upper" className="uitext" /> &gt; <Cmdname sid="instance_performance_analysis" className="uitext" /> 메뉴에서 <Cmdname sid="heap_histogram" className="uitext" /> 탭을 선택하세요.

  :::note

  **지원 범위**

  * 이 옵션은 Java 6 ~ 8 버전에서 지원합니다.

  * Java 5 버전 이하, IBM Java는 지원하지 않습니다.

  * Java 9 ~ Java 15 버전에서는 JVM 옵션(`-Djdk.attach.allowAttachSelf=true`)을 추가해야 합니다.

    ```bash
    java -javaagent:{WHATAP_HOME}/whatap.agent-X.Y.Z.jar -Djdk.attach.allowAttachSelf=true -jar {application.jar}
    ```
  
  * Java 16 버전 이상은 다음의 JVM 옵션을 적용해야 합니다.
