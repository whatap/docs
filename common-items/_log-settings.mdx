...
## ![](/img/number-02.png) 로그 모니터링 시작하기{#log-getting-started}
...
## ![](/img/number-03.png) 로그 모니터링 데이터 설정{#log-data-setting}

![number-s](/img/number-03.png) 상단에서 ***로그 사용량***을 확인할 수 있습니다. 또한 ***데이터 유지 기간*** 및 ***로그 조회 비밀번호*** 설정을 변경할 수 있습니다.

### 데이터 유지 기간{#data-retention}

공통으로 적용할 기본(default) 데이터 유지 기간입니다. 미지정 시 기본값은 1일입니다. 카테고리별 데이터 유지 기간을 별도로 설정하지 않으면 이 데이터 유지 기간이 기본적으로 적용됩니다. 카테고리별 데이터 유지 기간을 설정하고 ***리셋*** 버튼을 선택하면 기본 데이터 유지 기간으로 초기화됩니다.
...
## ![](/img/number-04.png) 로그 파서 설정{#log-setting-parser}
...
1. ![number-s](/img/ic-number-07.svg) 영역 ![아이콘](/img/ico-filter.svg)***필터*** 버튼을 선택해 필터를 추가할 수 있습니다. 필터 입력 시 필터 조건에 맞는 로그 데이터에만 패턴을 적용합니다.
...
1. 시뮬레이션 결과가 정상적이라면 ![number-s](/img/ic-number-08.svg) 영역 ***저장*** 버튼을 선택해 저장하세요.

:::note

* ![number-s](/img/ic-number-07.svg) ***필터***를 추가하는 것은 선택 사항입니다.
* ![number-s](/img/ic-number-08.svg) 영역 ***리셋*** 버튼을 선택하면 작성 중 내용이 초기화됩니다.
* ![number-s](/img/ic-number-09.svg) 버튼을 선택하고 파서 설정을 드래그해 순서를 변경할 수 있습니다.
...
### Grok 패턴 파서 등록{#log-pattern-grok}
...
로그 중 JSON 형태로 출력된 부분을 검출하기 위하여 ***Prefix***, ***Postfix*** 옵션을 조합해 로그의 어느 부분을 JSON으로 인식해 파싱할지 지정합니다. 
...
### 상태 코드 성공률 파서 패턴 등록{#log-pattern-status}
...
## ![](/img/number-05.png) 빠른 인덱스 설정{#log-setting-index}
...
## ![](/img/number-06.png) 로그 장기 보관 통계 설정{#log-long-term-archive}
