...
| Field | Unit | Shortname/Name/Description |
| -- | -- | -- |
...
| ^ | ^ | 컨테이너 메모리 최대 사용량 기록값 <br/> 자세한 내용은 [하단 안내](#scrollTo-memmaxusage)를 참조하세요. |
...
| Tags | Description | Type | Additional Info |
|---|---|---|---|
...
| Field | Unit | Description | Type | Additional Info |
| --- | --- | --- | --- | --- |
| cpu | pct(%)| CPU 사용률||float|/proc/[pid]/stat 을 통해 계산된 필드|
| memory | pct(%)|메모리 사용률|float|/proc/[pid]/statm 을 통해 계산된 필드|
| rss | Bite | 실제 메모리 사용량 (Resident Set Size) |long|/proc/[pid]/status::VmRSS|
| uid | - | 사용자 ID 또는 사용자 이름| String |/proc/[pid]/status::Uid|
| state | - |프로세스 상태|String|/proc/[pid]/status::State|
| SharedMemory | Bite |공유 메모리 크기|long|/proc/[pid]/statm 을 통해 계산된 필드|
| openFileDescriptors | - |프로세스가 열어둔 파일 디스크립터 수|int|/proc/[pid]/fd를 통해 계산된 필드|
| vmSize | Bite | 가상 메모리 크기 (Virtual Memory Size) |long|/proc/[pid]/status의 VmSize|
...
|코드| Description | Additional Info |
|---|---|---|
...
| Field | 단위 | Description | Additional Info |
| ---- | :--: | ---- | ---- |
...
| Tags | Description |단위|타입|
|---|---|---|---|
|`deployName`| 디플로이먼트의 이름 (`metadata.name`) | - | String |
|`namespace`| 디플로이먼트가 속한 네임스페이스 (`metadata.namespace`) | - | String |
|`deployUid`| 디플로이먼트의 UID (`metadata.uid`) | - | String |

### Fields {#kube_deployment-fields}

| Field | Description |단위|타입|
|---|---|---|---|
|`deployReadyReplicas`| 준비(Ready) 상태인 파드 수 (`status.readyReplicas`) | 개수 | - |
|`deployTotalReplicas`| 설정된 전체 파드 수 (`spec.replicas`) | 개수(count) | - |
|`deployUpdatedReplicas`| 최신 버전으로 갱신된 파드 수 (`status.updatedReplicas`) | 개수 | - |
|`deployAvailableReplicas`| 사용 가능한 상태의 파드 수 (`status.availableReplicas`) | 개수 | - |
|`deployUnavailableReplicas`| 사용 불가능한 상태의 파드 수 (`status.unavailableReplicas`) | 개수 | - |
|`deployCreationTime`| 디플로이먼트 생성 시각 (Unix Epoch milliseconds, `metadata.creationTimestamp`) | 밀리초(ms) | - |
|`deploySelector`| 디플로이먼트의 선택자 라벨 (`spec.selector.matchLabels`) | - | String |
|`deployStrategy`| 디플로이먼트의 배포 전략 (`spec.strategy.type`, 예: `RollingUpdate`, `Recreate`) | - | String |
...
| Tags | Description |타입|
|---|---|---|
|`cronJobName`| CronJob의 이름 (`metadata.name`) | String |
|`namespace`| CronJob이 속한 네임스페이스 (`metadata.namespace`) | String |
|`cronJobUid`| CronJob의 UID (`metadata.uid`) | String |

### Fields {#kube_cronjob-fields}

| Field | Description |Unit| 타입|
|---|---|---|---|
|`lastScheduleTime`| CronJob이 마지막으로 스케줄된 시간 (`status.lastScheduleTime`) | - | String |
|`lastSuccessfulTime`| CronJob이 마지막으로 성공한 시간 (`status.lastSuccessfulTime`) | - | String |
|`cronJobSpecSchedule`| CronJob의 스케줄 주기 (`spec.schedule`, 예: `"0 */5 * * *"` ) | - | String |
|`successfulJobsHistoryLimit`| 성공한 Job을 보존하는 최대 개수 (`spec.successfulJobsHistoryLimit`) | 개수 | - |
|`failedJobsHistoryLimit`| 실패한 Job을 보존하는 최대 개수 (`spec.failedJobsHistoryLimit`) | 개수 | - |
|`concurrencyPolicy`| 동시 실행 정책 (`Allow`, `Forbid`, `Replace`) (`spec.concurrencyPolicy`) | 문자열 | - |
|`startingDeadlineSeconds`| 스케줄된 시간이 지난 후에도 Job을 실행할 수 있는 최대 지연 시간 (`spec.startingDeadlineSeconds`) | 시간(seconds) | - |
...
| Tags | Description | Unit |타입|
|---|---|---|---|
|`jobName`| Job의 이름 (`metadata.name`) | - | String |
|`namespace`| Job이 소속된 네임스페이스 (`metadata.namespace`) | - | String |
|`jobUid`| Job의 UID (`metadata.uid`) | - | String |
|`ownerKind`| 상위 오브젝트의 종류 (예: `CronJob`) | - | String |
|`ownerName`| 상위 오브젝트의 이름 | - | String |
|`ownerUid`| 상위 오브젝트의 UID | - | String |

### Fields {#kube_job-fields}

| Field | Description | Unit| 타입|
|---|---|---|---|
|`succeededPods`| 성공한 파드 수 (`status.succeeded`) | 개수 | - |
|`failedPods`| 실패한 파드 수 (`status.failed`) | 개수 | - |
|`completionTime`| Job이 완료된 시간 (`status.completionTime.toString()`) | - | String |
|`parallelism`| 동시에 실행되도록 설정된 파드 수 (`spec.parallelism`) | 개수 | - |
|`completions`| Job이 완료되기 위해 필요한 파드 실행 횟수 (`spec.completions`) | 개수 | - |
|`backoffLimit`| 실패 시 재시도 가능한 최대 횟수 (`spec.backoffLimit`) | 횟수 | - |
|`activeDeadlineSeconds`| Job이 실행될 수 있는 최대 시간 (`spec.activeDeadlineSeconds`) | 초(seconds) | - |
