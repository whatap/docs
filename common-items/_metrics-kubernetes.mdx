...
|uid|사용자 ID 또는 사용자 이름|없음|String|/proc/[pid]/status::Uid|
|state|프로세스 상태|없음|String|/proc/[pid]/status::State|
|createTime|프로세스 시작 시간|타임스탬프|long|/proc/[pid]/uptim 을 통해 계산된 필드|
|SharedMemory|공유 메모리 크기|바이트 (B)|long|/proc/[pid]/statm 을 통해 계산된 필드|
|openFileDescriptors|프로세스가 열어둔 파일 디스크립터 수|없음|int|/proc/[pid]/fd를 통해 계산된 필드|
|vmSize|가상 메모리 크기 (Virtual Memory Size)|바이트 (B)|long|/proc/[pid]/status의 VmSize|
|threads|프로세스가 생성한 스레드 수|없음|int|/proc/[pid]/status의 Threads|
