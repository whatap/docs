...
| desired_pod                  | 정수      | `metadata.ownerReferences` 없이 배포된 Pod 수                                        | -                      |
| ^                            | ^         | 쿠버네티스 오브젝트(ReplicaSet, Daemonset, StatefulSet)에 정의된 Desired Pod 의 수 | -                      |
...
| ^ | ^ |Pod 메모리 할당량 기준 working set 사용량<br/>= mem_working_set / mem_limit * 100 |
