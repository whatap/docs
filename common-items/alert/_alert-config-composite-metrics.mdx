...
| 조건 | 이벤트 발생 조건 |
| 이벤트 상태가 해소되면 추가 알림 | <span className="vslow">Critical</span>과 <span className="slow">Warning</span> 레벨의 이벤트가 해소되면 RECOVERED(정상) 상태의 알림 수신 여부 <br /> – 활성화: 진행 중 → 정상 상태로 전환되면 RECOVERED 알림을 보내고, 이벤트 기록에서 진행 중/해소 상태를 구분해 볼 수 있음 <br /> – 비활성화: 임곗값을 초과할 때마다 단발성 이벤트 발생 |
...
import Alerteventsharing from "../../common-items/alert/_alert-event-sharing.mdx";
