---
id: db-agent-network
title: DB 에이전트 네트워크 통신
description: 데이터베이스 에이전트의 네트워크 통신에 관한 설정입니다.
tags: [ 데이터베이스, 데이터베이스 모니터링, 에이전트, 네트워크, 통신 ]
---

에이전트를 설치한 데이터베이스 서버와 와탭 수집 서버 사이에 네트워크 통신을 위한 설정들입니다.

* **license** <span class='type'>String</span><br/>
에이전트를 설치할 때 서버로부터 부여받은 프로젝트 액세스 키를 설정합니다. 프로젝트 액세스 키는 에이전트가 속한 프로젝트와 보안 통신을 위한 암호 키를 포함하고 있습니다.

* **whatap.server.host** <span class='type'>String</span><br/>기본값 `127.0.0.1,127.0.0.1`<br/>
에이전트가 수집한 데이터를 전송할 와탭 수집 서버의 IP 주소를 설정하세요. 수집 서버 이중화로 2개 이상의 IP를 가진 경우 쉼표(,)를 구분자로 사용하세요. 설정한 IP 주소에는 수집 서버 proxy 데몬이 리스닝 상태로 서비스돼야 합니다.

* **whatap.server.port** <span class='type'>String</span><br/>기본값 `6600`<br/>
수집 서버 포트(port)를 설정합니다. 포트는 하나만 설정할 수 있으므로 `whatap_server_host`에 설정한 수집 서버들은 동일 포트를 사용해야 합니다.

* **dbms** <span class='type'>String</span><br/>
모니터링할 데이터베이스의 종류를 설정합니다. DB의 종류는 다음 중 한 가지를 설정할 수 있습니다. 
  * `oracle`
  * `postgresql`
  * `mysql`
  * `mssql`

* **db_ip** <span class='type'>String</span><br/>
모니터링할 데이터베이스 서버의 IP 주소를 설정합니다.

* **db_port** <span class='type'>Int</span><br/>
데이터베이스가 통신에 사용하는 포트를 설정합니다.각 데이터베이스별 기본 포트는 다음과 같습니다.
  * oracle: `1521`
  * postgresql: `5432`
  * edb: `5444`
  * mysql: `3306`
  * mssql: `1433`

* **db** <span class='type'>String</span><br/>
데이터베이스 접속에 필요한 db 명을 설정합니다. 
  * oracle의 경우 SID를 설정합니다.
  * postgresql과 edb의 경우 설정하지 않으면 기본값으로 postgres의 db를 사용합니다.
  * mssql의 경우 설정하지 않으면 기본값으로 master의 db를 사용합니다.
  * mysql의 경우 설정하지 않아도 됩니다.