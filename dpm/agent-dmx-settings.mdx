...
* **license** <span class='type'>String</span>  
  에이전트를 설치할 때 서버로부터 부여받은 **프로젝트 액세스 키**를 설정합니다.  
  액세스 키에는 에이전트가 속한 프로젝트 정보와 보안 통신용 암호 키가 포함되어 있습니다.

* **whatap.server.host** <span class='type'>String</span>  
  기본값: `127.0.0.1,127.0.0.1`  
  에이전트가 데이터를 전송할 **와탭 수집 서버의 IP 주소**를 설정합니다.  
  수집 서버 이중화 시 쉼표(,)로 구분해 여러 IP를 설정할 수 있습니다.  
  설정한 IP에는 수집 서버 proxy 데몬이 리스닝 상태여야 합니다.

* **whatap.server.port** <span class='type'>String</span>  
  기본값: `6600`  
  수집 서버 포트를 설정합니다.  
  포트는 하나만 지정할 수 있으며, `whatap.server.host`에 설정한 모든 서버는 동일한 포트를 사용해야 합니다.

* **ipckey** <span class='type'>String</span>  
  모니터링할 오라클의 Key 값(16진수)을 설정합니다.  
  `ipcs -m` 명령어를 사용해 값을 확인할 수 있습니다.  

  :::note  
  `ipcs` 명령은 프로세스 간 통신(IPC) 정보를 조회합니다.  
  `-m` 옵션은 공유 메모리 자원을 표시합니다.  
  :::
