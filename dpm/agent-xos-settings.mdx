...
- XOS 에이전트는 **x86 아키텍처 기반 OS 환경**에서만 동작합니다.  

- **CUBRID** 환경에서는 [이 문서](install-agent#apply-cubrid-xos-agent)를,  
  그 외 데이터베이스 환경에서는 [이 문서](install-agent#apply-xos-agent)를 참조하세요.
