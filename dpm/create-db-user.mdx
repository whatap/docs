...
id: create-db-user
title: 모니터링용 DB 계정
description: 데이터베이스 모니터링을 위해 필요한 권한을 가진 계정을 생성하는 방법을 설명합니다.
keywords: [ Database, Account, DBUser, Permission ]
---

데이터베이스 모니터링을 하기 위해서는 필요한 권한을 가진 계정이 필요합니다.  


:::note
- 이미 권한을 가진 계정이 있다면 이 단계를 생략하고 [액세스 키 확인](#check-accesskey)으로 이동하세요.  
- 예시 코드에서 `DB_User` 또는 `whatap`은 DB 사용자 계정 이름이며, 실제 사용하는 계정명으로 변경하세요.  
- 예시 코드의 `{DB_Password}`에는 실제 비밀번호를 입력하세요.  
:::

### Altibase

root(admin) 계정 또는 관리자 권한으로 로그인하여 모니터링용 계정을 생성하세요.
...
* 멀티 테넌트 DB에서 kill session 기능을 사용할 경우
  * `c##` 계정으로 실행
  * Oracle Only 
