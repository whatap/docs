...
Spring 프레임워크에서는 `org.springframework.web.servlet.DispatcherServlet.processHandlerException` 메소드를 통과하는 경우 에러로 처리합니다. 다만 개발자가 Try-Catch 문을 사용하는 경우 `ExceptionHandler`에 의해 처리되므로 와탭에서는 에러 처리하지 않습니다.


#### Status Code 400 이상 에러 처리

HTTP 응답 코드가 400 이상인 경우에는 예외 클래스(Exception Class)가 명시되지 않더라도 에러로 처리합니다.
