...
>```
>:::note
>MySql 8.0 이상 `caching_sha2_password` 플러그인 문제 해결에 대한 자세한 내용은 [다음 문서](https://kogle.tistory.com/87)를 참조하세요.
>:::
...
>A. 에이전트는 db 정보를 하루에 한 번 그리고 기동 시 한 번 수집합니다. 에이전트가 떠있는 상태에서 db를 추가 생성한 경우 반영이 안 될 수 있습니다. 이 경우 먼저 에이전트 재기동을 수행합니다. 모니터링되지 않는다면 적절한 권한이 없어서일 수 있습니다. 다음과 같이 권한을 부여하세요. 
>```sql
