...
> DB 모니터링에서 CPU, Memory 등 리소스 지표가 보이지 않아요.

DB 서버의 리소스 데이터를 수집하려면 환경에 따라 다음 방법을 확인하세요.
* **DB Agent 환경**: DB Local 서버에 XOS 모듈을 설치해야 리소스 데이터를 수집할 수 있습니다. 자세한 설정 방법은 [다음 문서](../mysql/agent-xos-settings)에서 참고하세요.

* **RDS 환경**: RDS는 Local 서버에 설치되지 않는 환경이므로 AWS CloudWatch와 연동하여 관련 메트릭을 수집할 수 있습니다. 자세한 설정 방법은 [다음 문서](../reference/cloudwatch-metric-guide)를 참고하세요.
