...
>A. 에이전트는 db 정보를 실행 시 한 번 그리고 하루에 한 번 더 수집합니다. 에이전트가 실행 중인 상태에서 db를 추가 생성한 경우 반영되지 않을 수 있습니다. 에이전트를 다시 시작하세요. 그래도 해결되지 않는다면 적절한 권한이 없어서일 수 있습니다. 다음과 같이 권한을 부여하세요. 
>```sql
