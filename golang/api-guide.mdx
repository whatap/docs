...
`Sart()`, `StartWithRequest()` 함수에 `context`를 전달 및 호출하면 `context` 내부에 `whatap` 키로 `traceCxt`가 생성되고, `Txid(int64)` 고유 아이디가 생성됩니다. 
...
다음 설정을 통해서 HTTP 파라미터, HTTP 헤더 정보를 수집할 수 있습니다.
...
* `trace.Step()`
  ...
  사용자가 전달하는 문자열을 프로 파일 정보에 출력하는 기능을 제공합니다.
