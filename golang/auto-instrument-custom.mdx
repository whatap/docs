...
이 문서는 **고급 사용자**를 위한 내용입니다. 대부분의 경우 [기본 사용법](auto-instrument)과 [설정 가이드](auto-instrument-config)만으로 충분합니다.
...
| 방법 | 설명 | 주요 사용 사례 |
|------|------|----------------|
| [add](#add---새-파일함수-생성) | 새 파일/함수 생성 | 헬퍼 함수 추가 |
| [inject](#inject---함수-정의-내부에-코드-삽입) | 함수 내부에 코드 삽입 | 모든 함수에 trace 추가 |
| [replace](#replace---함수-호출-교체) | 함수 호출 교체 | `sql.Open` → `whatapsql.Open` |
| [hook](#hook---함수-호출-전후에-코드-삽입) | 함수 호출 전후 코드 | 레거시 코드 계측 |
| [transform](#transform---코드-패턴-변환) | 패턴 변환 (고급) | 복잡한 변환 |
