...
id: auto-instrument
title: 기본 사용법
description: whatap-go-inst 도구로 코드 수정 없이 Go 애플리케이션에 모니터링을 추가하는 방법을 안내합니다.
keywords: [ Go, whatap-go-inst, 자동 설정 ]
---

Go 모니터링 자동 적용은 와탭에서 개발한 **whatap-go-inst** 도구를 사용합니다. `whatap-go-inst`는 빌드 시 모니터링 코드를 자동으로 삽입하는 CLI 도구로, 소스코드를 직접 수정할 필요가 없습니다.

`whatap-go-inst` 설치를 위해 아래 **지원 환경**을 확인한 후 **빠른 시작** 단계를 확인하세요. 

:::info 다른 설정 방법

API를 직접 사용하여 모니터링 코드를 추가하려면 [수동 설정 가이드](manual-instrument.mdx)를 참고하세요.
...
whatap-go-inst 바이너리 자체는 Go 설치 없이도 실행 가능합니다. 단, 계측된 코드를 빌드하려면 Go 1.18 이상이 필요합니다.
...
이제 [WhaTap 모니터링 서비스](https://service.whatap.io)에서 데이터를 확인할 수 있습니다.

:::

<details>
<summary>💡 원본 코드를 전혀 변경하고 싶지 않다면?</summary>
...
`whatap-go-inst`는 빌드 시 소스코드를 분석하여 모니터링 코드를 자동으로 삽입합니다.
...
<details>
<summary>전체 지원 라이브러리 목록 보기</summary>

| 분류 | 지원 라이브러리 |
|------|----------------|
| **웹 프레임워크** | Gin, Echo v4, Fiber v2, Chi v5, Gorilla Mux, net/http, FastHTTP |
| **데이터베이스** | database/sql, sqlx, GORM v1/v2 |
| **Redis** | go-redis v8/v9, Redigo |
| **NoSQL** | MongoDB |
| **메시지큐** | Kafka (Sarama - IBM/Shopify) |
| **RPC** | gRPC |
...
|    구분    | 명령어                    | 설명                             | 주요 사용   |
| :--------: | ------------------------- | -------------------------------- | ----------- |
| **초기화** | `whatap-go-inst init`     | 프로젝트 초기화 및 의존성 추가   | 최초 1회    |
|  **기본**  | `whatap-go-inst go build` | 모니터링 코드를 삽입하여 빌드    | 일반 빌드   |
|  **기본**  | `whatap-go-inst go run`   | 모니터링 코드를 삽입하여 실행    | 개발/테스트 |
|  **기본**  | `whatap-go-inst go test`  | 모니터링 코드를 삽입하여 테스트  | 단위 테스트 |
|  **고급**  | `whatap-go-inst inject`   | 계측 코드를 별도 디렉토리에 출력 | 코드 검토   |
|  **고급**  | `whatap-go-inst remove`   | 삽입된 모니터링 코드 제거        | 계측 해제   |
|  **유틸**  | `whatap-go-inst version`  | 버전 확인                        | 버전 체크   |
...
| 옵션 | 설명 |
|------|------|
| `--verbose, -v` | 상세 출력 (파일별 변환 내용 표시) |
| `--quiet, -q` | 요약만 출력 |
| `--config` | 설정 파일 경로 (기본: `.whatap/config.yaml`) |
...
원본을 전혀 변경하고 싶지 않다면 `--wrap` 모드를 사용하세요.

### 기존 go 명령어와 무엇이 다른가요?

`whatap-go-inst go build`는 일반 `go build`와 동일하게 작동하지만, 빌드 전에 모니터링 코드를 자동으로 삽입합니다. 모든 옵션과 플래그는 그대로 사용할 수 있습니다.
```bash
# 일반 빌드 명령어와 동일하게 사용
whatap-go-inst go build -o myapp -ldflags="-s -w" .
```
...
초기 빌드는 약간 느려질 수 있지만, Go의 빌드 캐시 덕분에 이후 빌드는 거의 영향이 없습니다.
...
더 자세한 내용은 [설정 가이드](auto-instrument-config.md#트러블슈팅)를 참고하세요.
...
- [설정 가이드](auto-instrument-config.mdx) - 명령어 옵션, 설정 파일, Preset
- [사용자 정의 가이드](auto-instrument-custom.mdx) - 제한사항, 트러블슈팅, 고급 사용법
- [멀티 모듈 가이드](auto-instrument-multi-module.mdx) - 멀티 모듈 프로젝트 계측
- [수동 설정 가이드](manual-instrument.mdx) - API 직접 사용 방법
- [설치 가이드](install-agent.mdx) - WhaTap Go 에이전트 설치
- [Docker 설치 가이드](install-agent-docker.mdx) - Docker 환경 설치