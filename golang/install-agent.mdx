...
Alpine Linux의 경우 다음과 같이 스크립트를 실행해 안내에 따라 **액세스 키**와 **수집 서버 IP 주소**를 입력하세요.
...
### whatap.conf 파일 경로를 `WHATAP_HOME` 환경 변수로 설정하기

`WHATAP_HOME` 경로를 우선 생성해야 합니다.
...
### 동일한 애플리케이션을 두 개 이상의 프로세스로 실행하는 경우
