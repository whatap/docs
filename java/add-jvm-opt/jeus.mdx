...
[와탭 모니터링 서비스](https://service.whatap.io/)에 로그인한 다음 프로젝트를 선택하세요. ***애플리케이션*** > ***인스턴스 성능 분석*** 메뉴를 선택한 다음 ***실행 환경 변수*** 탭을 선택하세요. ***whatap.name***과 ***whatap.type*** 항목을 확인하세요. ***whatap.name*** 항목의 마지막 요소가 컨테이너 이름이어야 합니다. ***whatap.type*** 항목에는 애플리케이션 서버의 종류가 명시되어야 합니다.
