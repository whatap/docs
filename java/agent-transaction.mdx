...
* **hook_HttpURLConnection_startup_enabled** <span class='type'>Boolean</span>

  기본값 `false`

  `HttpURLConnection` 클래스의 메소드를 최초 호출 시에도 추적할 수 있도록 설정할 수 있습니다.

* **HttpURLConnection_weblogic** <span class='type'>Boolean</span>

  기본값 `true`

  Weblogic에서 `HttpURLConnection`으로 http 호출을 하는 경우 `weblogic.net.http.HttpURLConnection` 클래스의 메소드를 추적하기 위한 옵션입니다.

* **hook_HttpURLConnection_weblogic_startup_enabled** <span class='type'>Boolean</span>
