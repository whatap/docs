...
* <Cmdname sid="agent_configuration" className="uitext" />
  ...
  에이전트 설정 기능은 사용성과 기능을 개선한 <Cmdname sid="management" className="uitext" /> &gt; <Cmdname sid="agent_configuration" className="uitext" /> 메뉴를 이용할 것을 권장합니다. 자세한 내용은 [다음 문서](set-agent#set-agent-service)를 참조하세요.

  :::

* <Cmdname sid="agent_log" className="uitext" />

  모니터링 대상 서버에 저장된 에이전트 로그를 조회할 수 있습니다. 로그 파일의 이름은 *whatap-`YYYYMMDD`.log* 형식입니다.