...
에이전트 자동 삭제를 위해 와탭 서버는 자바 에이전트로부터 메시지를 받아야 합니다. 에이전트는 `${WHATAP_HOME}`에 Java 프로세스의 *whatap_{process id}.shutdown* 파일이 나타나면 **SILENT_SHUTDOWN** 이벤트를 와탭 서버에 전송합니다. 전송은 5초 이내로 완료합니다.
