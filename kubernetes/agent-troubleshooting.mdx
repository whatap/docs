...
이미 동일한 이름의 다른 리포지토리가 존재할 경우 다음과 같이 기존 whatap 리포지토리 삭제 후 설치를 진행하세요.
...
클러스터에 `whatap-monitoring` 네임스페이스가 존재한다면 사용자가 *yaml* 파일 기준으로 이미 와탭 쿠버네티스 에이전트를 설치했기 때문입니다.

이 경우 다음 명령어를 통해 기존 `whatap-monitoring` 네임스페이스 및 관련 리소스를 삭제하세요.
