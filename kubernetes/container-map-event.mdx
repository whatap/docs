...
홈 화면 > 프로젝트 선택 > ***대시보드*** > ***컨테이너 맵*** > 대상 선택 후 **상세 보기** 화면 진입 > ***이벤트*** 탭 선택
...
![쿠버네티스 상세 보기 이벤트 탭](/img/k8s-event-default2.png)

* ![number 1](/img/number-01.png) 쿠버네티스 이벤트 또는 와탭 이벤트를 선택해 조회할 수 있습니다. 그룹 및 블록에 관한 자세한 설명은 [다음 문서](container-map#container-map-chart)를 참조하세요.
  ...
  * 그룹 또는 개별 블록 선택
    ...
    ***컨테이너 맵***의 뷰에 따라 제공하는 그룹화 속성 또는 개별 블록 선택 시 조회할 수 있습니다. 그룹화에 관한 자세한 내용은 [다음 문서](container-group)를 참조하세요.

  :::note

  **상세 보기** 화면에 진입하기 전 ***컨테이너 맵*** 상단에서 선택한 뷰 및 그룹화 속성에 따라 조회할 수 있는 이벤트가 달라집니다.

  :::

* ![number 2](/img/number-02.png) 시간 선택자를 통해 조회를 원하는 시간대를 지정할 수 있습니다. 기본 설정은 ***1일***입니다.

* ![number 3](/img/number-03.png) 바 차트를 통해 선택한 시간대의 ***이벤트 수 추이***를 살펴볼 수 있습니다.

* ![number 4](/img/number-04.png) 이벤트 목록을 통해 이벤트 정보를 확인할 수 있습니다. 
  ...
  * ***Kubernetes*** 이벤트
    ...
    * 각 이벤트의 좌측 띠 역시 이벤트 타입을 나타냅니다. 회색은 <span class='k8s-normal'>Normal</span>, 주황색은 <span class='slow'>Warning</span> 상태를 의미합니다.
  ...
  * ***WhaTap*** 이벤트
    ...
    * 각 이벤트의 좌측 띠 역시 이벤트 레벨을 나타냅니다. 회색은 <span class='k8s-normal'>Info</span>, 주황색은 <span class='slow'>Warning</span>, 빨간색은 <span class='vslow'>Critical</span> 상태를 의미합니다.
...
각 이벤트의 좌측 띠는 이벤트 타입을 나타냅니다. 회색은 <span class='k8s-normal'>Normal</span>, 주황색은 <span class='slow'>Warning</span> 상태를 의미합니다. 사용자 선택 대상에 따른 제공 쿠버네티스 이벤트는 다음과 같습니다.
...
각 이벤트의 좌측 띠는 이벤트 레벨을 나타냅니다. 회색은 <span class='k8s-normal'>Info</span>, 주황색은 <span class='slow'>Warning</span>, 빨간색은 <span class='vslow'>Critical</span> 상태를 의미합니다.
...
와탭 이벤트는 `message`, `status`, `title`, `level` 등의 항목으로 구성되어 있습니다.
