...
홈 화면 > 프로젝트 선택 > ***대시보드*** > ***컨테이너 맵*** > 대상 선택 후 **상세 보기** 화면 진입 > ***트레이스*** 탭 선택
...
와탭 쿠버네티스의 트레이스 기능은 컨테이너 상에서 서비스 중인 애플리케이션 정보를 불러와 해당 애플리케이션의 트랜잭션 현황을 모니터링합니다.
...
***트레이스*** 탭의 하위 메뉴는 ***대시보드***, ***트랜잭션 목록***으로 구성되어 있습니다.

## 대시보드

대시보드는 다음과 같은 애플리케이션의 핵심 정보를 한눈에 제공합니다. 선택한 컨테이너 혹은 Pod, 그룹 내에서 서비스 중인 애플리케이션의 현황을 실시간으로 파악할 수 있습니다. 액티브 트랜잭션, 히트맵, TPS 등의 차트를 확인할 수 있습니다. 대시보드를 구성하고 있는 차트 및 지표에 대한 안내는 다음과 같습니다.
...
***트랜잭션 목록*** 화면은 지정된 시간대의 트랜잭션 목록 정보를 제공합니다. 목록에서 항목을 선택하면 해당 트랜잭션의 속성과 성능 정보 및 수행 이력을 확인할 수 있습니다. 만약 트랜잭션 분포도를 확인하여 특정 구간의 트랜잭션들만 확인하려면 위 트레이스 ***대시보드***의 히트맵 차트에서 대상을 선택(드래그)하세요.
 
![트랜잭션 목록](/img/k8s-tx-list.png)

<!-- 

1. 트랜잭션 목록 실시간 조회 가능한지?
실시간 시간 표기는 있으나
버그인지 부하로 막아둔 것인지 히스토리 확인 필요

2. v2 트레이스 분석으로 동선 교체할 것인지?
