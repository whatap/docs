...
사용자 애플리케이션 실행 경로에 와탭의 액세스 키, 서버 IP 주소 정보를 입력한 *whatap.conf* 파일을 생성하세요. 특정 경로를 사용할 경우 `WATAP_HOME` 환경변수로 설정할 수 있습니다.
...
*whatap.conf* 파일 경로를 `WHATAP_HOME` 환경변수로 설정할 수 있습니다. `WHATAP_HOME` 경로를 우선 생성하세요. 애플리케이션 실행 문구 앞에 `WATAP_HOME` 설정을 추가합니다. 
...
애플리케이션 시작 명령어 앞에 추가로 whatap-agent 서비스 시작 명령어를 추가하세요. 
