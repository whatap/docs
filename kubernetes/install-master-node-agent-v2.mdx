...
액세스 키 확인 후 ***설치 안내*** 화면에서 ***설치 방식 선택*** 탭을 선택하세요. 와탭 쿠버네티스 에이전트가 지원하는 **Helm** 차트와 **Yaml** 설치 방식 중 원하는 설치 방식을 선택 후 사용자의 쿠버네티스 환경에 따라 알맞은 항목을 선택하세요. 

<ImgLang img='k8s-install-master-node-method.png' desc='설치 방식 선택'/>

1. ***Helm*** 차트와 ***Yaml*** 중 원하는 **설치 방식**을 선택하세요. 
...
**컨트롤 플레인 모니터링**에 대한 자세한 내용은 [다음 문서](https://docs.whatap.io/kubernetes/kube-apiserver-dashboard)를 참조하세요. 
...
와탭 쿠버네티스 에이전트 설치 시 **Helm** 차트 기준 설치 방식 안내와 **Yaml** 파일 기준 설치 방식 안내를 다음과 같이 제공합니다. 
