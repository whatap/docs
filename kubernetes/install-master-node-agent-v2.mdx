...
와탭 쿠버네티스 에이전트는 기존 DockerHub 외에 Public ECR을 추가로 지원합니다. 사용 환경에 따라 다음 두 이미지 리포지토리 중 하나를 선택하여 설치할 수 있습니다.

#### Public ECR

* 리포지토리 <code class='my'>추천</code>: `public.ecr.aws/whatap/kube_mon`

AWS ECR은 이미지 다운로드에 대한 제한이 없으므로, 빈번한 업데이트나 대규모 노드 배포 환경에서 안정적으로 사용할 수 있습니다.

#### DockerHub

* 리포지토리: `whatap/kube_mon`

DockerHub는 인증되지 않은 클라이언트의 경우 pull 제한이 발생할 수 있습니다. 노드 수가 많거나 동시에 pull 요청이 발생하는 경우 rate limit 에 걸릴 수 있습니다. 다음 [DockerHub 정책 안내](https://docs.docker.com/docker-hub/usage/)를 참고해 주세요.

:::note 

pull 제한이 발생한 경우 다음 문서(개별 문서 또는 안내 추가)를 참고하세요.
