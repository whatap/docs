...
와탭 에이전트 설치 *YAML* 파일에서 `ClusterRole` 리소스의 `rules` 하위의 `resources` 항목에 사용하려는 `customResource`를 추가하세요. 
...
Helm을 사용하여 설치할 경우 기본 *values.yaml* 파일에 모니터링할 Custom Resource 이름을 추가하세요. 다음과 같이 `resources` 항목에 해당 Custom Resource 이름을 추가하세요. 
