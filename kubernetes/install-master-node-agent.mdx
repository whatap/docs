...
와탭 쿠버네티스 에이전트 설치 시 *yaml* 파일 기준 안내와 및 *helm* 차트 기준 안내를 다음과 같이 제공합니다.
...
1. 다음 명령어를 실행해 에이전트 애플리케이션을 설치하세요. 
...
*values.yaml* 파일을 이용한 주요 구성 수정 방법의 예시는 다음과 같습니다. 
...
1. **차트 디버깅** 실제 애플리케이션 배포 시 문제 발생 여부를 점검합니다.
...
1. **인덱스 파일 생성** 현재 디렉터리에 있는 Helm 차트로부터 인덱스 파일을 생성합니다. 
...
<details>
<summary><b>Helm 차트 구조</b></summary>

와탭이 제공하는 *helm* 차트의 구조는 다음과 같습니다.
