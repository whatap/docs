...
사용자는 <Cmdname sid="TTL08456" className="uitext" />을 통해 노드 상태를 색상으로 표현할 수 있습니다. 지표는 1개 이상 설정할 수 있으며 각 지표별로 정상(<span class='normal'>Normal</span>), 경고(<span class='slow'>Warning</span>), 오류(<span class='vslow'>Error</span>) 구간을 설정하여, 해당 범위에 속할 때 지정된 색상이 적용됩니다. 

1. <Cmdname sid="TTL08456" className="uitext" /> 옵션을 클릭하세요. 

1. 원하는 지표를 선택 후 각 <span class='normal'>Normal</span>, <span class='slow'>Warning</span>, <span class='vslow'>Error</span> 구간을 설정하세요. 
...
각 상태 구간인 <span class='normal'>Normal</span>, <span class='slow'>Warning</span>, <span class='vslow'>Error</span>는 기본적으로 <span class='normal'>파란색</span>, <span class='slow'>주황색</span>, <span class='vslow'>빨간색</span>으로 설정되어 있습니다. <Cmdname sid="TTL08456" className="uitext" />에서 각 상태에 대한 색상을 사용자가 지정할 수 있습니다.
...
색상 우선 순위는 <span class='vslow'>빨간색</span>으로 표시되는 <span class='vslow'>Error</span> 구간이 가장 높습니다. 여러 지표 중 하나의 항목이라도 <span class='vslow'>Error</span> 구간에 속할 경우 해당 노드는 우선적으로 <span class='vslow'>빨간색</span>으로 표시됩니다.
...
사용자는 노드 블록 내부에 텍스트 라벨을 설정하여 지정한 주요 정보를 한눈에 볼 수 있습니다.
...
* <span class='normal'>Normal</span>: 값이 `0` 초과 `80` 이하
...
노드 맵 우측 상단의 ![설정 아이콘](/img/ic-setting.svg) 아이콘을 통해 추가적인 세부 설정을 조정할 수 있습니다.
...
노드 맵 차트는 다음 세 가지 영역으로 구성되어있습니다. 

![지시선 1](/img/number-01.png) **위젯 상단 영역**: 노드의 전체 개수 또는 그룹화된 경우 그룹회된 항목들의 총 개수와 선택한 메트릭스의 상태별 개수를 표시합니다. 

![지시선 2](/img/number-02.png) **중앙 시각화 영역**: 전체 노드 상태와 개수를 직관적으로 확인할 수 있는 시각화된 대시보드를 제공합니다. 

![지시선 3](/img/number-03.png) **위젯 하단 영역**: 선택한 메트릭스의 통계값을 표시합니다. 추가 설정의 <Cmdname sid="BTN08460" className="uitext" />을 통해 설정한대로 합계, 최대값, 최소값 중 원하는 방식으로 데이터를 표시합니다. 
...
<Cmdname sid="side_nodeMap" className="uitext" />맵에서 조회를 원하는 노드를 선택하세요. 오른쪽에 나타나는 요약 보기 화면을 통해 해당 객체의 요약 정보를 확인할 수 있습니다. 요약 보기 화면에서 상세 분석 버튼을 클릭해 상세 분석 화면을 확인할 수 있습니다. 
...
***Labels*** 지표는 마스터 에이전트 버전 1.7.9 이상부터 지원합니다.
