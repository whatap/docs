...
와탭 **노드 에이전트 버전 1.4.2 이상**에서 지원합니다.

:::

Pod가 안정적으로 동작하기까지의 성능을 분석하고 Pod에 할당된 리소스 크기를 최적화할 필요가 있습니다. <Cmdname sid="side_podInitPerf" className="uitext" /> 메뉴는 Pod가 시작한 후 약 5분 동안의 변화를 추적합니다. **Pending** 상태에서 **Running** 상태로, 그리고 안정적인 **Stable Running** 상태로 이어지는 3단계 구간의 자원 사용량을 분석해 리소스 할당량을 최적화할 수 있습니다.

이를 통해 운영자는 자원 효율성 향상 및 비용 절감, 확장성 향상, 스케일 용이성 등의 이점을 얻을 수 있습니다.

Pod 생성일로부터 최근 30일 이내의 범위 안에서 Pod 시작 데이터(5분)를 분석합니다. Pod 생성 후 30일이 지난 Pod의 경우 시작 성능을 분석할 수 없습니다.
