...
와탭 **노드 에이전트 버전 1.4.2 이상**에서 지원합니다. **1.4.2 미만**인 경우 기존 [Pod 초기화 성능](pod-init-perform) 메뉴를 제공합니다. 
...
***Pod 시작 분석*** 메뉴는 Pod 생성일로부터 최근 30일 이내의 범위 안에서 Pod 5분간의 시작 데이터를 분석합니다. 즉 Pod 생성 후 30일이 지난 경우 Pod의 시작 성능을 분석할 수 없습니다.
