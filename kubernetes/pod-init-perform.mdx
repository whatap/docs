...
* ![number 01](/img/number-01.png) 영역 좌측에서 ![새로고침 아이콘](/img/ic-k8s-filter.svg) 컬럼 아이콘을 선택해 컬럼을 선택할 수 있습니다. 

* ![number 01](/img/number-01.png) 영역 우측 입력창에서 ***Pod 이름*** 또는 ***QoS 클래스***를 검색할 수 있습니다.
...
***초기화 시작***은 최초 메트릭스 데이터가 수집된 시점을 의미합니다.

* 초기화 시작 시 CPU Throttle: 초기화 시작 시 컨테이너에서 CPU 제한을 초과하는 사용 시도 발생 시 증가하는 값을 의미합니다.

* 초기화 시작 시 CPU Limits: 초기화 시작 시 CPU limit을 의미합니다.

* 초기화 시작 시 CPU 요청량: 초기화 시작 시 CPU request를 의미합니다.  

* 초기화 시작 시 Memory Throttle: 초기화 시작 시 컨테이너에서 메모리 제한을 초과하는 사용 시도 발생 시 증가하는 값을 의미합니다.

* 초기화 시작 시 Memory Limits: 초기화 시작 시 설정한 메모리 limit을 의미합니다.

* 초기화 시작 시 Memory Requests: 초기화 시작 시 요청한 메모리 request를 의미합니다. 

* 컨테이너 재시작 수: Pod에 존재하는 컨테이너의 재시작 수의 합을 의미합니다. 
...
***초기화 중***은 초기화 시작 후 5분간 데이터의 CpuByLimit 평균 값보다 낮은 CpuByLimit 값의 데이터가 발생한 이전 시점을 의미합니다. 

* 초기화 중 평균 CpuByLimit: 초기화 중 사용한 CpuByLimit의 평균 값을 의미합니다. 

* 초기화 중 최대 메모리: 초기화 중 해당 컨테이너에서 사용한 메모리 최대 사용량을 의미합니다. 

* 초기화 소요된 시간(초): Pod 초기화에 소요된 시간을 의미합니다. 
...
***초기화 완료***는 초기화 시작 후 5분간 데이터의 CpuByLimit 평균 값보다 낮은 CpuByLimit 값의 데이터가 발생한 시점을 의미합니다.

* 초기화 완료 후 평균 CpuByLimit: 초기화 완료 후 사용한 CpuByLimit의 평균 값을 의미합니다.

* 초기화 완료 후 최대 메모리: 초기화 완료 후 해당 컨테이너에서 사용한 메모리 최대 사용량을 의미합니다. 

* 초기화 완료 후 (밀리초)(5분까지): 초기화 완료 후 5분 동안의 밀리초 단위 시간을 의미합니다. 

* 초기화 완료 시간: 초기화가 완료된 시점에 대한 데이터를 의미합니다. 