...
* **collect_apiserver_metric_by_leader** <span class='type'>bool</span>
  ...
  해당 옵션이 활성화 된 경우 노드 에이전트는 메트릭 수집을 마스터 에이전트에 위임합니다. whatap-master-agent의 `collect_apiserver_metric_by_leader` 옵션 활성화가 필요합니다. 메트릭 수집 위임을 통해 기존 whatap-node-agent가 발생시킨 apiserver 부하를 낮출 수 있습니다.
...
* **debug** <span class='type'>bool</span>
  ...
  whatap-master-agent에서 광범위한 debug log 수집 여부를 설정합니다. 
...
* ***대시보드*** > ***컨테이너 맵***
  ...
  Pod 그룹화 시 화면 데이터 갱신 주기에 영향을 받습니다. 다음 예시 이미지와 같이 통계 데이터는 `kube_pod` 메트릭스 수집 주기 및 수집 서버 전송 주기에 영향을 받습니다. 

* ***워크로드*** > ***Pod 목록***

  다음 예시 이미지와 같이 통계 데이터는 `kube_pod` 메트릭스 수집 주기 및 수집 서버 전송 주기에 영향을 받습니다.
...
* ***대시보드*** > ***컨테이너 맵*** 
  ...
  컨테이너 그룹화 시 표시되는 화면 데이터 갱신 주기에 영향을 받습니다. 
...
* ***클러스터*** > ***노드 상세*** 
  ...
  ***노드 상세*** 화면에서 ***컨테이너*** 목록의 경우 갱신 주기에 영향을 받습니다.
