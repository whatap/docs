---
id: log-basics
title: 작동 원리
description: 로그 모니터링 서비스의 작동 원리를 안내합니다. 
tags: [ 로그 모니터링 ]
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

![log-wt-vs](/img/log-wt-vs.svg)

<!-- 상단 비교화면 디자인팀 요청~구조도 검토 필요 -->
일반적인 로그 통합 서비스는 수집기, 처리기, 저장소 그리고 UI 모듈로 이루어져 있습니다. 단계별 설정과 구성 작업을 필요로 하기에 각각의 모듈을 구축하는 과정이 번거로우며 추가 비용이 발생합니다.

**와탭 로그 모니터링은 적용이 간단합니다**. 기존의 모니터링 에이전트가 수집기 역할을 하기에 에이전트 **옵션을 켜는 것**만으로 로그 모니터링을 시작할 수 있습니다.

<Tabs>
<TabItem value="java" label="Java" default>

* 출력된 파일에서 로그를 읽지 않고 Java 애플리케이션의 로그 라이브러리로 전달되는 로그를 직접 수집합니다.  

* 로그를 직접 수집하기 때문에 파일 I/O를 유발하지 않아 시스템에 미치는 성능 영향이 매우 낮습니다.

* 트랜잭션 프로파일과 로그의 연결 추적성을 확보하여 프로파일에서 로그를 확인할 수 있습니다.

:::note Java 로그 라이브러리
대표적인 Java 로그 라이브러리는 Apache Log4j, Logback
:::

:::caution
Java Agent 2.1.0 버전부터 사용할 수 있습니다.
:::
</TabItem>
<TabItem value="php" label="PHP" default>

* {@include: _log-basics-default.mdx}  

:::caution
PHP Agent 2.3.2 버전부터 사용할 수 있습니다.
:::
</TabItem>
<TabItem value="python" label="Python" default>

* {@include: _log-basics-default.mdx}  

* 로그에 트랜잭션 ID를 출력하면, 트랜잭션 프로파일과 로그의 연결 추적성을 확보하여 프로파일에서 로그를 확인할 수 있습니다.  

:::caution
Python Agent 1.2.2 버전부터 사용할 수 있습니다.
:::
</TabItem>
<TabItem value="server" label="Server" default>

* {@include: _log-basics-default.mdx}   

:::caution
Server Agent 2.1.2 버전부터 사용할 수 있습니다.
:::
</TabItem>
</Tabs>