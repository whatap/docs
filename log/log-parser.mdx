...
tags: [ 로그, 로그 모니터링, 설정하기, 로그 파서, 로그 파싱 ]
---
...
로그가 불규칙한 형태로 수집되는 경우 GROK 파서를 사용해 로그를 파싱할 수 있습니다. GROK 문법은 named regular expressions을 제공해 정규 표현식을 보다 쉽고 편리하게 사용할 수 있는 문법을 제공합니다.
...
위의 [샘플 로그](#syntax-semantic-usage)에서 `%{SYNTAX:SEMANTIC}` 문법만으로 파싱되지 않는 영역이 있습니다. 

```javascript title='미파싱 영역'
...
1. ***+ 추가하기***를 선택 후 ***파서*** 입력란에서 ***GROK*** 파서를 선택하세요.

  <ImgLang img='log-1st-grok.png' desc='GROK 파서 패턴 등록'/>

1. ***카테고리*** 및 ***로그 검출 조건***, ***패턴***을 입력하세요. ***파서 추가*** 창의 구성 요소는 다음과 같습니다. 
...
1. ***파서 시뮬레이션*** 창에서 ***로그***와 ***패턴***을 입력하세요. 

1. ***로그***와 ***패턴*** 입력 후 ***시뮬레이션*** 버튼을 선택하세요. 다음과 같이 ***시뮬레이션 결과***를 확인할 수 있습니다.
...
1. ***+ 추가하기***를 선택 후 ***파서*** 입력란에서 ***JSON*** 파서를 선택하세요.

  <ImgLang img='log-1st-JSON.png' desc='GROK 파서 패턴 등록'/>
...
1. ***카테고리*** 및 ***로그 검출 조건***, ***패턴***을 입력하세요. ***파서 추가*** 창의 구성 요소는 다음과 같습니다. 
    ...
    * ***패턴***
      ...
      * JSON 패턴 예시  
        ```javascript title='예시 로그'

        2023-08-08 02:43:28,615 -- {"host":"10.21.3.24","method":"POST","status":"200","url":"http://devote.whatap.io/yard/api/flush"} --

        ```

        예시 로그에서 ***Prefix***와 ***Postfix***를 `--`로 지정하고 ***Ignore***에 `--`를 지정 시 `host`, `method`, `status` 3개의 ***태그***만 생성됩니다.
...
* 파서를 등록하기 전에 ***시뮬레이션***을 통해 등록하려는 패턴이 정상적인지 확인할 수 있습니다. GROK 시뮬레이션 과정과 동일합니다. [다음 문서](#grok-simulation)를 참조하세요.
