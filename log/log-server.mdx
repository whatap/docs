...
```bash title='Linux Shell' showLineNumbers {6-11}
cd /usr/whatap/infra
...
[[inputs.logsink]]
  category = "serverlog" 
  stats_enabled = true ## 로그 발생량 통계 별도 데이터로 전송 여부
  stats_category = "logsink_stats" ## 로그 발생량 통계 카테고리
...
```bash title='Windows Powershell (관리자 권한)' showLineNumbers {5-10}
...
excludeNames = [ ".gz",".zip" ] ## 로그 파일 경로(path)에 별표()가 포함되어 제외할 로그 비대상 파일명 설정
[[inputs.logsink.file]]
_path = "c:\whatap\logs%Y-%m-%d_.log" ## 로그 파일 지정 시, 날짜 패턴(strftime.org) 지정 가능
disabled = false
encoding = "euc-kr"
