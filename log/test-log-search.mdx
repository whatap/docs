...
로그에서 파싱되지 않은 즉 인덱스가 생성되지 않은 키워드를 포함한 로그를 조회할 수 있습니다. Content 검색의 경우 지정 범위 내 모든 로그를 Full Scan합니다. 그렇기 때문에 인덱스가 생성된 키와 비교해 검색 속도가 다소 떨어질 수 있습니다. 정형화된 로그 데이터의 경우 파서 설정을 통해 인덱스 키 값을 활용해 검색하는 것을 권장합니다. 파서 설정에 대한 자세한 내용은 [다음 문서](log-parser.mdx)를 참조하세요.
