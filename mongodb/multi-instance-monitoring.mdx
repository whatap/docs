...
* ***Read requests(/s)***
  ...
  초당 읽기 요청 수를 측정한 위젯입니다. **Read requests**는 데이터베이스에서 데이터를 조회하거나 검색하는 쿼리, 집계 작업, 인덱스 스캔 등과 관련한 작업입니다. 데이터베이스의 사용 패턴의 변화를 감지하는데 유용합니다.

* ***Write requests(/s)***

  초당 쓰기 요청 수를 측정한 위젯입니다. ***Write requests***는 데이터베이스에 새로운 데이터를 추가하거나 업데이트, 삭제와 관련한 작업입니다. 데이터베이스의 사용 패턴의 변화를 감지하는데 유용합니다.
