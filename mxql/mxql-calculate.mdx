...
| [JOIN](#join)               | 다른 MQL에서 조회한 데이터를 본 데이터에 칼럼단위로 추가할때 사용합니다. |
...
|okind |`okid` 컬럼의 이름을 설정하여 `okid` 값에 대응되는 `okind name`의 값을 가지는 컬럼을 생성합니다.|
|onode |`onode` 컬럼의 이름을 설정하여 `onid` 값에 대응되는 `onode name`의 값을 가지는 컬럼을 생성합니다.|
...
| -- | -- | 
| key | 정렬할 필드를 선택합니다. |
| sort | 정렬한 direction을 설정합니다. (asc 또느 desc) |
...
| over  | 특정 값보다 같거나 큰 데잍러를 찾습니다.(greater 또는 equal to) |
| under | 특정 값보다 같거나 작은 데잍러를 찾습니다.(less 또는 equal to) |
