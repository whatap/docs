...
![MXQL 데이터 조회](https://img.whatap.io/media/images/mxql-7.png)
...
1. **메트릭스 로딩**: 이전 단계에서 설정한 값들을 사용해서 메트릭스를 불러옵니다. 대부분의 경우 `TAGLOAD`를 이용하며 ~~[특수한 경우]~~에만 `FLEXLOAD`를 이용하세요.
...
* 수집서버에 데이터를 저장할 때부터 모든 필드에는 MetricValue 형식으로 저장된 카테고리가 있습니다. ***사이트맵*** > ***분석*** > ***메트릭스 조회*** > ***카테고리*** 옵션에는 ***기본***, ***5분***, ***1시간*** 단위로 선택할 수 있는 카테고리를 확인할 수 있습니다. 여기서 ***5분*** 또는 ***1시간***을 선택할 수 있는 카테고리가 MetricValue 형식으로 저장된 카테고리입니다.
  ...
  ![MetricValue 형식으로 설정할 수 있는 카테고리 목록](https://img.whatap.io/media/images/mxql-3.png)
...
link:./mxql_guide.html#_saas_서비스에서_제공하는_사전에_정의된_mxql_쿼리_목록[SaaS 서비스에서 제공하는 사전에 정의된 MXQL 쿼리 목록]에서 사용 가능한 쿼리를 확인할 수 있습니다.
...
.바인드 변수 설정 예시
image::https://img.whatap.io/media/images/mxql-1.png[]

NOTE: 바인드 변수의 이름은 대소문자 알파멧만 가능합니다. 숫자 및 특수문자는 바인드 변수의 이름에 포함될 수 없습니다. 

=== 데이터 로딩방식

MXQL에서 조회할 수 있는 데이터는 link:../metrics/metrics_overview.html[메트릭스]의 형식에 따라서 크게 두 가지로 나눌 수 있습니다.



1. 메트릭스의 데이터가 태그와 필드에 나누어서 저장된 데이터 ( link:./mxql_metrics_load.html#tagload[TAGLOAD]를 사용해서 로드할 수 있는 데이터)
2. 메트릭스에 모든 데이터가 필드에 저장된 데이터 (link:./mxql_metrics_load.html#flexload[FLEXLOAD]를 사용해서 로드할 수 있는 데이터)
