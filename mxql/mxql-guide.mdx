...
MXQL에서 조회하는 데이터는 [메트릭스](../metrics/common-metrics-intro)의 구조에 따라 서로 다른 로딩 명령어를 사용합니다.

- **태그 기반 메트릭스**  
  태그와 필드로 구분된 데이터는 [`TAGLOAD`](mxql-loading#tagload) 명령어를 사용해 로드합니다.

- **플랫 구조 메트릭스**  
  모든 데이터가 필드에 저장된 메트릭스는 [`FLEXLOAD`](mxql-loading#flexload) 명령어를 사용해 로드합니다.

- **로그 원본 데이터**  
  로그 원본을 조회할 때는 [`LogSink`](mxql-loading#logsink) 명령어를 사용합니다.

- **로그 카운트 집계 데이터**  
  시간 단위로 로그 건수를 집계할 때는 [`LogSinkCount`](mxql-loading#logsinkcount) 명령어를 사용합니다.

## 사전 정의된 MXQL 쿼리 목록{#custom-query-saas}
