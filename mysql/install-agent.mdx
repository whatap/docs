...
| replication | `grant replication client on *.* to whatap;` | <Cmdname sid="side_instanceList" className="uitext" /> 의 <UI>S</UI> 표시 / replication 정보 수집 | `SHOW MASTER STATUS`, `SHOW SLAVE STATUS` |
| \^ | `grant replication slave on *.* to whatap;` | <Cmdname sid="side_instanceList" className="uitext" />의 <UI>M</UI> 표시 / replication 정보 수집 | `SHOW SLAVE HOSTS` |
