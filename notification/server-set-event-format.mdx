...
이벤트 메시지에 포함될 수 있는 정보입니다. optional이 `false`인 경우 항상 메시지에 포함됩니다. optional이 `true`인 경우 해당 데이터를 확인할 수 있으면 보여줍니다.
...
| Server Name | 애플리케이션 이름 | `true` | oname|
...
| Alert Type | 이벤트 종류 | `true` | 이벤트 종류를 특정하기 위한 필드|
