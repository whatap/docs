...
와탭 서버의 수집 서버 세팅과 구동 및 관리 방법을 안내합니다. 이 안내 문서는 관리자 페이지를 이용한 운용 방안과 수집 서버 내 서버 구동 및 관리에 대해 설명합니다. 그리고 문제가 발생했을 때 체크해야하는 지표에 대해 설명합니다.
...
수집 서버의 시간을 동기화합니다. 서버 간 시간이 맞지 않으며 데이터 누락 및 통계 왜곡이 발생할 수 있으므로 반드시 시간 동기화 설정이 되어야 합니다.
...
* Keeper: `netstat -na | grep LISTEN | grep 6789`
* NotiHub: `netstat -na | grep LISTEN | grep 6500`
* Front: `netstat -na | grep LISTEN | grep 8080`
* Yard: `netstat -na | grep LISTEN | grep 6610`
* Yard: `netstat -na | grep LISTEN | grep 7710`
* Proxy: `netstat -na | grep LISTEN | grep 6600`
