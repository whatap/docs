...
수집된 메트릭을 PromQL로 조회하고, 결과를 차트(Graph)와 테이블(Table)로 시각화합니다.
...
**가장 많이 사용되는 함수**로, Counter 메트릭의 **초당 평균 증가율**을 계산합니다.
...
지정한 시간의 **최대/최소값**을 계산하는 시간 범위 함수입니다.
...
값이 지정한 **최대값이나 최소값을 넘지 않도록 제한**합니다.
...
메트릭 이름만 사용하면 수천 개의 시계열이 선택될 수 있습니다.
    ...
    - Table 뷰는 **실험실** → **MXQL 데이터 조회**에서 `>>`로 시작하여 PromQL을 조회할 수 있습니다.  
    예. `>> nginx_http_response_count_total{status="200"}`
...
- [PromQL 기본 문서](https://prometheus.io/docs/prometheus/latest/querying/basics/)
- [PromQL 함수 문서](https://prometheus.io/docs/prometheus/latest/querying/functions/)
- [PromQL 사용 예제](https://prometheus.io/docs/prometheus/latest/querying/examples/)
