...
실행시간이 5초이상 되는 플랜을 수집

multi_db_stat

- 기본값 : true
- multi db로 사용하는 경우 DB별로 지표를 수집할 지, 전체 인스턴스 단위로 sum해서 수집할 지 지정하는 옵션

ora_hidden_param

- 기본값 : false
- true로 설정하면 히든 파라미터 값도 수집합니다.

tables_exclude_sys  

- 기본값 : true
- table 사이즈 수집 시 system 테이블들을 제외합니다. tables_min_size(default 10MB) 이상인 테이블만 수집합니다.

xview_min

- 기본값 : 100 (centi sec) (1초). 단위:centi sec
- -1:off, 0: 수집된것 모두
- 지정된 값보다 긴 elpase 의 것만 처리 합니다.
