...
화면 위에서 조회 시간 및 대상을 선택하세요. <Cmdname sid="time_selector" className="uitext" />과 <Cmdname sid="instance" className="uitext" /> 옵션을 설정한 다음 ![검색 아이콘](/img/ico-btn-search.png) 버튼을 선택하세요. 필요한 경우 <Cmdname sid="filter" className="uitext" /> 옵션을 설정해 원하는 조회 결과를 필터링할 수 있습니다.

* ***Access Count***: 시간별 Full Scan 발생 횟수를 확인할 수 있는 막대 그래프 차트입니다. 특정 시간대를 선택하면 선택한 시간에 수행된 SQL 목록이 화면 아래 목록과 오른쪽 ***Operation Count*** 차트에 표시됩니다.
