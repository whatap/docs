...
* **whatap.stat_enabled** <span class='type'>Boolean</span>
  ...
  통계 정보 추적 기능을 활성화합니다. 5분 단위로 수집되는 트랜잭션, SQL, HTTPCALL, UserAgent, Client IP 등의 통계 데이터 등이 해당됩니다.
...
* **whatap.stat_ipurl_enabled** <span class='type'>Boolean</span>
  ...
  프로젝트 사이드 메뉴 중 <Cmdname sid="side_statistics" className="uitext" /> &gt; <Cmdname sid="side_statIp" className="uitext" /> 통계 수집 여부를 설정합니다. 

  :::note 

  [다음 문서](/php/analysis-apm-trs#statiptx)를 참조하세요.
...
* **whatap.use_env_first** <span class='type'>Boolean</span> <code className="my">PHP agent v2.8.2 or later</code>
  ...
  에이전트 설정의 우선순위는 *whatap.ini* 파일이 먼저 적용되고, 그다음으로 환경 변수가 적용됩니다. 환경 변수를 우선적으로 적용하려면 옵션값을 `true`로 설정하세요. 또는 `WHATAP_USE_ENV_FIRST=true` 형식으로 환경 변수를 설정할 수도 있습니다.