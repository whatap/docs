...
주요 테이블을 파악하고 아케텍쳐, 백업 설계 등의 주요 대상을 검토하세요.
...
`xid_age`를 낮추기 위해서는 Vacuum 수행을 검토하세요. 자세한 내용은 [다음 문서](https://www.postgresql.org/docs/current/routine-vacuuming.html#VACUUM-FOR-WRAPAROUND)를 참조하세요.
...
| `schemaname` | 스케마(schema) 이름 |
| `tablename` | 테이블 이름 |
| `xid_age` | vacuum 작업으로 정리된 가장 오래된 `xid`의 간격(나이) |
| `table_size` | 테이블 사이즈 |
