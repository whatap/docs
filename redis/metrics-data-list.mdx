...
title: Redis 지표
description: Redis에서 사용하는 중요 지표를 안내합니다.
tags: [ Redis, 메트릭스, 데이터베이스, 데이터베이스 모니터링 ]
---

Redis에서 사용하는 중요 지표를 안내합니다. Redis 지표에 대한 자세한 내용은 [다음 문서](https://redis.io/commands/info/)를 참조하세요.
...
| `AOF_ENABLED` | *redis.conf*에 appendonly 모드가 yes이면 1, no면 0 입니다. yes일 때는 레디스 서버 시작 시 *appendonly.aof*을 읽고, no이면 *dump.rdb* 파일을 읽습니다. | PERSISTENCE |
