...
데이터베이스 모니터링에서는 CPU 사용량과 관련해 `CPU(xos)`, `CPU(xos2)` 지표 두 가지를 제공하고 있었습니다. DBX 에이전트 1.7.0 버전 이상에서는 두 지표가 통합될 예정입니다. xos2 지표는 폐기하고 xos 지표는 백분율로 변경합니다.
...
* 다음 메뉴 경로에서 개별로 설정한 프리셋 중 `cpu(xos2)` 지표를 `cpu(xos)` 지표로 변경하세요.
...
* 3월 정기 서비스 배포(2024.3.27 18:00) 후 대시보드에서 `cpu(xos)` 지표가 보이지 않는다면 DBX 에이전트를 1.7.0 버전 이상으로 업데이트하세요. 에이전트 업데이트에 대한 자세한 내용은 [다음 문서](../postgresql/agent-manage)를 참조하세요.
