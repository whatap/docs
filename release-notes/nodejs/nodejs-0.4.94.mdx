...
- <Status>New</Status> <Link to="http://socket.io">socket.io</Link> 모니터링 모듈 추가

  `trace_sampling_enabled` ,`trace_sampling_tps` 옵션을 사용하여 샘플링된 데이터를 확인할 수 있습니다. `trace_ignore_url_prefix` 옵션에 `/socket.io` 경로를 추가하여 트레이스하지 않도록 설정하세요.
