...
| ------------------- | ------------------------------------ | ------------------------------------------ |
| script-src          | https://repo.whatap.io/rum/prod/     | 브라우저 에이전트 CDN 도메인               |
| connect-src         | https://rum-ap-northeast-2.whatap.io | 수집한 데이터의 전송을 위한 도메인         |
| script-src          | unsafe-inline                        | 브라우저 에이전트를 인라인 스크립트로 설치 |
| worker-src          | blob:                                | 브라우저 에이전트가 웹워커를 사용하기 위함 |
| child-src           | blob:                                | 브라우저 에이전트가 웹워커를 사용하기 위함 |

## Timing-Allow-Origin 설정

리소스를 제공하는 서버에 CORS를 적용하면 대부분의 리소스 타이밍 속성을 0으로 반환합니다. 와탭 브라우저 에이전트가 리소스의 속성 값을 정상적으로 수집하려면 서버에서 `Timing-Allow-Origin` HTTP 응답 헤더를 반환해야 합니다. 
