...
서버 구성 정보를 대량으로 입력해야 하는 경우 개별 데이터를 수작업으로 입력하는 데 많은 시간이 소요됩니다. *CSV* 파일 업로드 기능을 통해 대량 데이터 입력을 효율적으로 처리하세요.
...
* *CSV* 업로드 시 <Cmdname sid="TTL08183" className="uitext" /> 및 커스텀 컬럼(<code class='custom-cl'>custom</code>)의 변경 사항만 적용합니다. 
