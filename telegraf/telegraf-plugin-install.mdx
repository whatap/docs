...
1. 실행 파일을 빌드합니다. *${GOPATH}/bin* 디렉토리에 실행 파일(telegraf)이 생성됩니다.
    ...
    # make go-install
    ```
1. 설치 패키지가 필요하면 `make package` 명령어로 생성할 수 있습니다. 크로스 컴파일을 진행하여 운영체제별로 패키지 파일을 생성합니다.
    ```bash title=SH
    # make package
    ```

:::note
상세한 빌드 설명은 [Telegraf github](https://github.com/influxdata/telegraf) 내용을 참고해 주세요
:::
