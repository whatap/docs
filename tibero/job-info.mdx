...
id: job-info
title: Job 수행내역
description: Tibero에서 실행된 작업(Job)에 대한 정보를 제공합니다.
keywords: [ Job 수행내역 ]
---

홈 화면 &gt; 프로젝트 선택 &gt; <Cmdname sid="side_reportStat" className="uitext" /> &gt; <Cmdname sid="side_dbJobInfo" className="uitext" />

Tibero에서 실행된 작업(Job)에 대한 정보를 제공합니다. 화면 상단의 <UI>Job</UI> 섹션에서는 전체 Job 목록 및 관련 정보를 표시하며, 하단의 <UI>Job History</UI> 섹션에서는 상단에서 선택한 Job의 실행 이력을 상세히 확인할 수 있습니다. 이를 통해 Job 관리 및 실행 상태를 모니터링할 수 있습니다.
...
* **Job**은 **Tibero Agent**에 의해 관리되는 작업 단위로, 특정 작업(예: 데이터베이스 백업, 데이터 정리, 쿼리 실행 등)을 자동으로 수행하도록 구성된 프로세스를 말합니다. **Job**은 관리자가 반복적이거나 일정한 간격으로 실행해야 하는 작업을 자동화하는 데 유용합니다. 자세한 내용은 [다음 링크](https://learn.microsoft.com/sql/ssms/agent/create-a-job?view=sql-server-ver16)를 참조하세요.

* Job은 Tibero 2016 버전 이상부터 지원하고, Express 버전은 지원하지 않습니다.
...
Tibero에서 등록된 Job의 기본 정보를 제공합니다. Job의 생성/수정 시간, 실행 상태 및 알림 설정 등을 확인할 수 있습니다. 제공하는 컬럼 항목에 대한 자세한 내용은 다음을 참조하세요.
...
* **run_duration**: 단계가 마지막으로 실행되었을 때의 시간(hhmmss)입니다.
